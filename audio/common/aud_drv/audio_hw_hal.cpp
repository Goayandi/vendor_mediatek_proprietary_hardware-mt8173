...
public:
    ...
    inline ~AudioAutoTimeProfile() {
        ...
        if (mTotalTime >= mThresholdTime) {
            ALOGD("mtkAudProfile %lld ms @ %s", mTotalTime/1000000, s8.string());
            if (mDumpStack > 0) {
                CALLSTACK();
            }
        }
...
static uint32_t audio_device_conv_table[][HAL_API_REV_NUM] =
{
    ...
    { AUDIO_DEVICE_IN_MATV, AUDIO_DEVICE_IN_MATV},
#endif
};
...
err_open:
                                      ...
                                      audio_source_t source /*__unused*/)
    {
       ...
       //devices = convert_audio_device(devices, HAL_API_REV_2_0, HAL_API_REV_1_0);
        ...
        in->legacy_in = (AudioMTKStreamInInterface*)ladev->hwif->openInputStreamWithFlags(devices, (int *) &config->format,
                                                     &config->channel_mask, &config->sample_rate,
                                                     &status, (AudioSystem::audio_in_acoustics)0, flags);
#else
        in->legacy_in = (AudioMTKStreamInInterface*)ladev->hwif->openInputStream(devices, (int *) &config->format,
                                                     &config->channel_mask, &config->sample_rate,
