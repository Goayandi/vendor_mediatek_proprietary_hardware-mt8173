...
#else
        ALOGE("%s(),  AUDIO_C2K_SUPPORT is not defined!!!", __FUNCTION__);
        pCCCI = new SpeechMessengerECCCI(mModemIndex, this);
...
#if !defined(MTK_AUDIO_BT_NREC_WO_ENH_MODE) && !defined(MTK_AURISYS_PHONE_CALL_SUPPORT)
...
status_t SpeechDriverLAD::SetDynamicSpeechParameters(const int type, const void *param_arg)
{
    ...
    int u4I, size_byte;
    char *pPackedSphParamUnit = new char [3456];
    ...
    switch (type)
    {
        case AUDIO_TYPE_SPEECH:
            memset(pPackedSphParamUnit, 0, 3456);
...
status_t SpeechDriverLAD::GetNxpSmartpaParam(void *eParamNxpSmartpa)
{
    ...
    {
        pParamNxpSmartpa->mic_index = MIC_INDEX_O27;
    }
...
#if defined(SMART_PA_SUPPORT) || defined(MTK_SPEAKER_MONITOR_SPEECH_SUPPORT)
