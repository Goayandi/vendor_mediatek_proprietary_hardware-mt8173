...
#define KERNEL_BUFFER_SIZE_WITH_DRE  (40 * 1024)
        if (mStreamAttributeTarget.buffer_size < KERNEL_BUFFER_SIZE_WITH_DRE)
        {
...
status_t AudioALSAPlaybackHandlerNormal::close()
{
    ...
    if (mHardwareResourceManager->getHeadPhoneChange() == true)
        mHardwareResourceManager->HpImpeDanceDetect();
#endif
    ALOGD("-%s()", __FUNCTION__);
    return NO_ERROR;
}
...
status_t AudioALSAPlaybackHandlerNormal::setScreenState(bool mode, size_t buffer_size, size_t reduceInterruptSize, bool bforce)
{
