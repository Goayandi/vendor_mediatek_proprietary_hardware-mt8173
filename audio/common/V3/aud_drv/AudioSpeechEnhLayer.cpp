...
SPELayer::~SPELayer()
{
    ...
    {
        hDumpThread = NULL;
        if (mDumpExit_Cond.waitRelative(mDumpExitMutex, milliseconds(timeoutMs)) != NO_ERROR)
        {
            ALOGD("%s, dumpthread close timeout?", __FUNCTION__);
        }
        mDumpExitMutex.unlock();
        DumpBufferClear();
    }
...
bool    SPELayer::SetFrameRate(SPE_MODE mode, long frame_rate)
{
    ...
    if (frame_rate != 10 && frame_rate != 20 && frame_rate != 5 && frame_rate != 3)
    {
...
bool    SPELayer::WaitforDownlinkData(void)
{
    ...
    if (timeoutMs != 0)
    {
        if (mBuf_Cond.waitRelative(mBufMutex, milliseconds(timeoutMs)) != NO_ERROR)
        {
...
bool SPELayer::PrepareProcessData()
{
    ...
    if (mDLNewStart || (DLdataPrepareCount > 0)) //the first queue downlink buffer is not ready to play yet, only need to process uplink data, so add zero data on downlink
    {
        ...
        ALOGD("%s, DL data is not ready yet, size %d", __FUNCTION__, mDLInBufferQ.size());
    ...
    else    //when all data is ready, check the estimate time to let DL/UL could start together.
    {
        ...
        ALOGD("%s, diffnsec %llu, diffSample=%ld, diffBufLength=%d", __FUNCTION__, diffnsec, diffSample, diffBufLength);
#if 1
        while (diffBufLength > 0)
        {
...
void *DumpThread(void *arg)
{
    ...
    pSPEL->mDumpExitMutex.unlock();
    pSPEL->mDumpExit_Cond.signal();
    ALOGD("DumpThread exit!!");
    pthread_exit(NULL);
    return 0;
}
...
void SPELayer::Dump_Enalbe_Check(void)
{
    ...
    if (bflag)
    {
        ...
        DumpFileNameUpIn.appendFormat("%s%s", audio_dump_path, "SPEIn_Uplink");
        ...
        DumpFileNameDownIn.appendFormat("%s%s", audio_dump_path, "SPEIn_Downlink");
    ...
    if (bflag)
    {
        ...
        DumpFileNameUpPro.appendFormat("%s%s", audio_dump_path, "SPE_Uplink");
        ...
        DumpFileNameDownPro.appendFormat("%s%s", audio_dump_path, "SPE_Downlink");
    ...
    if (bflag)
    {
        ...
        DumpFileNameUpOut.appendFormat("%s%s", audio_dump_path, "SPEOut_Uplink");
        ...
        DumpFileNameDownOut.appendFormat("%s%s", audio_dump_path, "SPEOut_Downlink");
    ...
    if (bflag || bflag1)
    {
        ...
        DumpFileNameEPL.appendFormat("%s%s", audio_dump_path, "SPE_EPL");
...
void SPELayer::EPLTransVMDump()
{
    ...
    if (bflag || mVMDumpEnable)
    {
        ...
        if (bflag)
        {
            ...
            strcpy(mVMDumpFileName, filename.string());
        }
