    ...
    for(;;)
    {
        ...
        switch(command)
        {
            ...
            case OFFLOAD_CMD_DRAIN:
                ...
                {
                    drain = true;
                    ret = ::ioctl(mFd, OFFLOADSERVICE_WRITEBLOCK, 1);
                }
                if (list_empty(&offload_thread.offload_cmd_list) && !ret)
                {
...
void AudioALSAPlaybackHandlerOffload::set_pcmdump(int enable)
{
    ...
    ::ioctl(mFd, OFFLOADSERVICE_PCMDUMP, enable);
}
...
//#endif
...

    {
        ...
        int pcmindex = AudioALSADeviceParser::getInstance()->GetPcmIndexByString(keypcmOffloadPlayback);
        ...
        mOffloadPcmConfig.stop_threshold = ~(0UL);
        ...
        {
            if (pcm_start(mOffloadPcmOut) != 0)
            {
                ALOGE("%s(), pcm_start(%p) fail due to %s", __FUNCTION__, mOffloadPcmOut, pcm_get_error(mOffloadPcmOut));
            }
        }
        ASSERT(mOffloadPcmOut != NULL);
...
int AudioALSAPlaybackHandlerOffload::process_write()
{
    ...
    if (!ret)
    {
        if (mWriteBsbufSize >= offload_stream.fragment_size)
        {
            ...
            memmove(offload_stream.tmpbsBuffer, (char*)offload_stream.tmpbsBuffer + offload_stream.fragment_size, mRemainedBS);
