...
#define MAX_KERNEL_BUFFER_SIZE  (32 * 1024)
#define HIFI_96K_BUFFER_SIZE    (MAX_KERNEL_BUFFER_SIZE * 2)
#define HIFI_192K_BUFFER_SIZE   (MAX_KERNEL_BUFFER_SIZE * 4)

    if (mStreamAttributeTarget.sample_rate <= 48000)
    {
        mStreamAttributeTarget.buffer_size = MAX_KERNEL_BUFFER_SIZE;
    }
    else if (mStreamAttributeTarget.sample_rate > 48000 && mStreamAttributeTarget.sample_rate <= 96000)
    {
        if (mStreamAttributeTarget.buffer_size >= HIFI_96K_BUFFER_SIZE)
        {
            mStreamAttributeTarget.buffer_size = HIFI_96K_BUFFER_SIZE;
        }
    }
    else
    {
        mStreamAttributeTarget.buffer_size = HIFI_192K_BUFFER_SIZE;
    }
#endif
        ...
        if (dsm_handler != NULL)   // do processing
        {
            ...
            */
            retcode = mMlds_Interace_pointer.mlds_process_dl_buf(
                          ...
                          &dsm_handler,
                          NULL);
            //ALOGD("-mlds_process_dl_buf mDsmpcmOut = %p mDsmIvReadData = %p outSize = %d", mDsmpcmOut, mDsmIvReadData, outSize);
