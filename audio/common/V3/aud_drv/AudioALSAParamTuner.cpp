...
static void *Play_PCM_With_SpeechEnhance_Routine(void *arg)
{
    ...
    {
        ALOGE("%s(), Play_PCM_With_SpeechEnhance_Routine pAUDParamTuning = NULL arg = %x", __FUNCTION__, arg);
        return 0;
    }
    ...
    if (tmp == NULL)
    {
        ALOGE("%s(), Allocate tmp size = %d Fail", __FUNCTION__, PLAYBUF_SIZE * 2);
        ASSERT(0);
        return 0;
    }
    ...
    ALOGD("%s(), pthread_cond_signal(&pAUDParamTuning->mPPSExit_Cond), buffer size=%d", __FUNCTION__, PCM_BUF_SIZE);
...
static void *DMNR_Play_Rec_Routine(void *arg)
{
    ...
    while ((!pDMNRTuning->m_bDMNRThreadExit) && pOutFp)
    {
        ...
        if (pInFp)
        {
            ...
            numOfBytesPlay += cntR;
            ALOGV("%s(), Playback buffer, free:%d, read from :%d, total play:%d", __FUNCTION__, playBufFreeCnt, cntR, numOfBytesPlay);
        }
...
static void *DMNR_Play_Rec_ApSide_Routine(void *arg)
{
    ...
    if (pInFp) //open output stream for playback
    {
        ...
        {
            if (WAV_HEADER_SIZE != fread(&waveHeader, WAV_HEADER_SIZE, 1, pInFp))
            {
                ALOGW("%s(), [Dual-Mic] cannot read wave header successfully!", __FUNCTION__);
                return 0;
            }
        }
...
{
    ALOGV("%s(), playing:%d", m_bPlaying, __FUNCTION__);
    bool ret = false;
    pthread_mutex_lock(&mP2WMutex);
#if defined(MTK_DUAL_MIC_SUPPORT) || defined(MTK_AUDIO_HD_REC_SUPPORT)
    ALOGV("%s(), DMNR playing:%d", m_bDMNRPlaying, __FUNCTION__);
...
{
    ALOGD("%s(), mode:%d", mMode, __FUNCTION__);
    pthread_mutex_lock(&mP2WMutex);
    uint32_t ret = mMode;
    pthread_mutex_unlock(&mP2WMutex);
    return ret;
...
void AudioALSAParamTuner::updataOutputFIRCoffes(AudioTasteTuningStruct *pCustParam)
{
    ...
    else
    {
        ...
        if ((cmdType == (unsigned short)AUD_TASTE_START || cmdType == (unsigned short)AUD_TASTE_INDEX_SETTING) && sCustMedParam.select_FIR_output_index[mode] != index)
        {
            ...
            ALOGD("%s(), ori sph_out_fir[%d][0]=%d, ori sph_out_fir[%d][44]", __FUNCTION__, mode, sCustParam.sph_out_fir[mode][0], mode, sCustParam.sph_out_fir[mode][44]);
            ...
            ALOGD("%s(), new sph_out_fir[%d][0]=%d, new sph_out_fir[%d][44]", __FUNCTION__, mode, sCustParam.sph_out_fir[mode][0], mode, sCustParam.sph_out_fir[mode][44]);
        ...
        ALOGD("%s(), sph_out_fir[%d][0]=%d, sph_out_fir[%d][44]", __FUNCTION__, mode, sCustParam.sph_out_fir[mode][0], mode, sCustParam.sph_out_fir[mode][44]);
...
status_t AudioALSAParamTuner::setPlaybackVolume(uint32_t mode, uint32_t gain, uint32_t device)
{
    ...
    ALOGD("%s(), gain:%ld,mode:%ld, device=%ld", __FUNCTION__, gain, mode, device);
