...
static String8 keyDeviceConnect = String8("connect");
...
{
    ALOGD("%s setMode = %d",__FUNCTION__,mode);
    status_t status;
...
status_t AudioALSAHardware::setParameters(const String8 &keyValuePairs)
{
...
#ifdef MTK_AURISYS_PHONE_CALL_SUPPORT
    char aurisys_dsp_call_key_prefix[] = "AURISYS_SET_PARAM,DSP,PHONE_CALL";
    char aurisys_hal_call_key_prefix[] = "AURISYS_SET_PARAM,HAL,PHONE_CALL";
    if (strncmp(aurisys_dsp_call_key_prefix,
                keyValuePairs.string(),
                strlen(aurisys_dsp_call_key_prefix)) == 0 ||
        strncmp(aurisys_hal_call_key_prefix,
                        keyValuePairs.string(),
                        strlen(aurisys_hal_call_key_prefix)) == 0)
    {
    ...
    if (param.getInt(keyDeviceConnect, value) == NO_ERROR)
    {
        param.remove(keyDeviceConnect);
        if (value == 4 || value == 8)
        {
...
String8 AudioALSAHardware::getParameters(const String8 &keys)
{
...
#ifdef MTK_AURISYS_PHONE_CALL_SUPPORT
    if (param.get(String8("AURISYS_SET_PARAM_RESULT"), value) == NO_ERROR)
    {
        param.remove(String8("AURISYS_SET_PARAM_RESULT"));
        SpeechDriverOpenDSP *pSpeechDriver =
            static_cast<SpeechDriverOpenDSP *>(
                SpeechDriverFactory::GetInstance()->GetSpeechDriver());
        int retval = pSpeechDriver->GetRetvalSetParameter();
        returnParam.add(String8("AURISYS_SET_PARAM_RESULT"),
            (retval == 1) ? String8("SET_PASS") : String8("SET_FAIL"));
    }


    char aurisys_dsp_call_key_prefix[] = "AURISYS_GET_PARAM,DSP,PHONE_CALL";
    if (strncmp(aurisys_dsp_call_key_prefix,
                keys.string(),
                strlen(aurisys_dsp_call_key_prefix)) == 0)
    {
    ...
    {
    #if defined(SMART_PA_SUPPORT)
        if (AudioSmartPaController::getInstance()->isApSideSpkProtect())
        {
            value = "1";
        }
        else
...
size_t AudioALSAHardware::getInputBufferSize(uint32_t sampleRate __unused, int format __unused, int channelCount __unused)
{
                                        ...
                                        audio_patch_handle_t *handle)
{
    ...
    ALOGV("handlecheck %s handle [0x%x] current size %d",__FUNCTION__, *handle,mAudioHalPatchVector.size());
    ...
    for (i = 0; i < num_sources ; i++)
    {
        ...
        {
            ALOGV("device.hw_module %x", sources[i].ext.device.hw_module);
        ...
        {
            ALOGV("mix.hw_module %x", sources[i].ext.mix.hw_module);
            ALOGD("mix.handle %x", sources[i].ext.mix.handle);
            ALOGV("mix.usecase.stream %x", sources[i].ext.mix.usecase.stream);
            ALOGV("mix.usecase.source %x", sources[i].ext.mix.usecase.source);
        }
    ...
    for (i = 0; i < num_sinks ; i++)
    {
        ...
        else if (sinks[i].type == AUDIO_PORT_TYPE_MIX)
        {
            ...
            ALOGV("mix.usecase.stream %x", sinks[i].ext.mix.usecase.stream);
...
// default implementation is unsupported
status_t AudioStreamOut::getNextWriteTimestamp(int64_t *timestamp __unused)
{
    return INVALID_OPERATION;
}
#if 0
