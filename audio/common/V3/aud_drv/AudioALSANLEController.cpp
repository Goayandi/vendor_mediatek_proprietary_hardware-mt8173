...
#ifndef ENABLE_NLE_HW_CONTROL
#ifndef VERY_VERBOSE_LOGGING
#define VERY_VERBOSE_LOGGING
#endif
#endif
...
AudioALSAHyBridNLEManager::AudioALSAHyBridNLEManager()
{
    ...
    ALOGD("mHwAnalogGainMinValue = %u", mHwAnalogGainMinValue);
    ALOGD("mHwAnalogGainMuteDb = %d", mHwAnalogGainMuteDb);
    ALOGD("mHwAnalogGainMuteValue = %u", mHwAnalogGainMuteValue);
    ALOGD("mHwDigitalGainMaxDb = %d", mHwDigitalGainMaxDb);
    ALOGD("mHwDigitalGainMaxValue = %u", mHwDigitalGainMaxValue);
...
status_t AudioALSAHyBridNLEManager::setNleHwConfigByDb(int hpMaxGainDb, int eopGainDb)
{
    ...
    if (mStatus == AUDIO_HYBRID_NLE_MNG_STATE_RUNNING) {
        if (PLAYBACK_HANDLER_NORMAL == mActivePBHandler) {
            AudioALSANLECtrl* pCurNLECtrl = mNleCtrlOfPlaybackHandler[PLAYBACK_HANDLER_NORMAL];
...
status_t AudioALSAHyBridNLEManager::process(playback_handler_t ePlaybackHandlerType, void *buffer, size_t Byte)
{
    ...
    if (ePlaybackHandlerType != PLAYBACK_HANDLER_NORMAL) {
        ALOGE("[Err] %s %d", __FUNCTION__, __LINE__);
...
status_t AudioALSAHyBridNLEManager::addHwPathStream(audio_stream_hw_path eHwPath)
{
    ALOGV("%s", __FUNCTION__);
    audio_hybridnlemng_status mOldStatus, mCurStatus;
...
status_t AudioALSAHyBridNLEManager::removeHwPathStream(audio_stream_hw_path eHwPath)
{
    ALOGV("%s", __FUNCTION__);
    audio_hybridnlemng_status mOldStatus, mCurStatus;
...
status_t AudioALSAHyBridNLEManager::updateCurStatus(void)
{
    ...
    ALOGD("mNleSwitch %d mBypassNLE %d PBHCount %d mMode %d HwPathCnt %d PBH %d", mNleSwitch, mBypassNLE, mNleCtrlOfPlaybackHandlerCounter, mMode, mHwPathStreamCounter, mActivePBHandler);
    do {
        if (mNleSwitch == false) {
            ...
            if (mStatus == AUDIO_HYBRID_NLE_MNG_STATE_DISABLE) {
                ...
                if (mNleCtrlOfPlaybackHandlerCounter == 1 && mNleCtrlOfPlaybackHandler[PLAYBACK_HANDLER_NORMAL] != NULL) {
                    if (mActivePBHandler != PLAYBACK_HANDLER_NORMAL) {
                        mActivePBHandler = PLAYBACK_HANDLER_NORMAL;
                    }
                    ALOGV("ts %d -> %d", mStatus, AUDIO_HYBRID_NLE_MNG_STATE_RUNNING);
                    mStatus = AUDIO_HYBRID_NLE_MNG_STATE_RUNNING;
                } else {
                    break;
                }
            } else if (mStatus == AUDIO_HYBRID_NLE_MNG_STATE_RUNNING) {
                if (mNleCtrlOfPlaybackHandlerCounter == 0 || mNleCtrlOfPlaybackHandler[PLAYBACK_HANDLER_NORMAL] == NULL) {
    ...
    ALOGVV("PHC %d mMode %d HwCount %d BypassNLE %d", mNleCtrlOfPlaybackHandlerCounter, mMode, mHwPathStreamCounter, mBypassNLE);
    return NO_ERROR;
}
        ...
        do {
            if (checkIfGainTargeted() == true) {
                ...
                if (sysDiff >= NLE_WAIT_NLE_BYPASS_MAX_MS) {
                    ALOGW("waitForBypass TimeOut %d MS", sysDiff);
                    break;
                }
                usleep(1000);
            }
...
status_t AudioALSAHyBridNLEManager::getDbFromAnalogIdx(size_t AnalogIdx, int8_t* pDb)
{
    ...
    if ((AnalogIdx > mHwAnalogGainMinValue && AnalogIdx < mHwAnalogGainMuteValue) || AnalogIdx > mHwAnalogGainMuteValue) {
        ALOGE("[Err] %s LINE %d AnalogIdx %x Reserved", __FUNCTION__, __LINE__, AnalogIdx);
        return BAD_VALUE;
    }
...
status_t AudioALSAHyBridNLEManager::getDbFromDigitalIdx(size_t DigitalIdx, int8_t* pDb)
{
    ...
    if (DigitalIdx > mHwDigitalGainMaxValue) {
        ALOGE("[Err] %s LINE %d DigitalIdx %x Reserved", __FUNCTION__, __LINE__, DigitalIdx);
        return BAD_VALUE;
    }
...
status_t AudioALSAHyBridNLEManager::getGstepAndGnum(int32_t SPL_diff, size_t ZeroCnt, int32_t* pdGstep, int32_t* pdGnum)
{
    ...
    if (ZeroCnt == 0) {
        ...
        dGnum = (SPL_diff + dGstep)/dGstep;
#endif
    } else if (ZeroCnt >= absSPL_diff) {
        dGstep = 1; // 1dB
        dGnum = 1; // 1 times
    } else {
...
status_t AudioALSAHyBridNLEManager::checkAndSetGain(size_t zuframecount, AudioALSANLECtrl* pNleCtrl, NleHwInfo_t* pNleHwInfo, RingBuf* pRingBuf)
{
    ...
    if (zuframecount == 0 || pNleCtrl == NULL || pNleHwInfo == NULL || pRingBuf == NULL) {
        ALOGW("[Warn] zuframecount = %d, pNleCtrl = %p, pNleHwInfo = %p, pRingBuf = %p %s  Line%d ", zuframecount, pNleCtrl, pNleHwInfo, pRingBuf, __FUNCTION__, __LINE__);
        return INVALID_OPERATION;
    } else if (pNleCtrl->mMinPreviewFrameCount > zuframecount) {
        ALOGW("[Warn] zuframecount = %d, min = %d, %s  Line%d ", zuframecount, pNleCtrl->mMinPreviewFrameCount, __FUNCTION__, __LINE__);
        //return INVALID_OPERATION; We continue to apply NLE even it has risk that ZCE isn't enough
    }
    dRet = pNleCtrl->getMaxSPLandZeroCnt(pNleHwInfo, pRingBuf, zuframecount, &SPL_Db, &Zero_Cnt);
...
AudioALSANLECtrl::AudioALSANLECtrl(playback_handler_t ePlaybackHandlerType, const stream_attribute_t *pStream_Attribute_Source)
{
    if (pStream_Attribute_Source != NULL) {
        ...
        if (ePlaybackHandlerType == PLAYBACK_HANDLER_NORMAL) {
            ...
            ALOGD("MB %d TB %d CEF %d SKZ %d MPB %d PBDF %d Ramp2ZeroFC %d", mMinPreviewByte, mHwBufTotalFrame, mCheckHwExtraFrameCnt, mSkipHwZCEFrameCnt, mMinPreviewFrameCount, mPlaybackedFrameCnt, mRamp2ZeroFrameCnt);
        }
#if 0
...
status_t AudioALSANLECtrl::checkValidForInit(playback_handler_t ePlaybackHandlerType)
{
    if (mState != AUDIO_NLE_STATE_NONE) {
        ALOGE("[Err] NLE init invalid mState [%d]", mState);
        return INVALID_OPERATION;
        ...
        ALOGE("[Err] NLE init fail device 0x%x", mStreamAttribute.output_devices);
        return BAD_VALUE;
        ...
        ALOGE("[Err] NLE init fail ePlaybackHandlerType %d", ePlaybackHandlerType);
        return BAD_VALUE;
    } else if (mStreamAttribute.audio_format != AUDIO_FORMAT_PCM_16_BIT && mStreamAttribute.audio_format != AUDIO_FORMAT_PCM_8_24_BIT) {
        ALOGE("[Err] NLE init fail audio_format 0x%x", mStreamAttribute.audio_format);
        return BAD_VALUE;
    } else if (mStreamAttribute.audio_channel_mask != AUDIO_CHANNEL_IN_STEREO) {
        ALOGE("[Err] NLE init fail audio_channel_mask 0x%x", mStreamAttribute.audio_channel_mask);
        return BAD_VALUE;
    } else if (mStreamAttribute.num_channels > NLE_SUPPORT_CH_NUMBER_MAX) {
        ALOGE("[Err] NLE init fail num_channels %d", mStreamAttribute.num_channels);
        return BAD_VALUE;
    } else if (mStreamAttribute.sample_rate > NLE_SUPPORT_SAMPLING_RATE_MAX) {
        ALOGE("[Err] NLE init fail sample_rate %d", mStreamAttribute.sample_rate);
        return BAD_VALUE;
    } else if ((mStreamAttribute.buffer_size < mMinPreviewByte) && ePlaybackHandlerType == PLAYBACK_HANDLER_NORMAL) {
...
status_t AudioALSANLECtrl::getMaxSPLandZeroCnt(NleHwInfo_t* pNleHwInfo, RingBuf* pRingBuf, size_t zuframecount, float* pSPL_Db, size_t* pZeroCnt)
{
    ...
    if (mBytePerSample == sizeof(int32_t)) {
        ...
        *pSPL_Db = 20.0*log10(fMaxSPL); // 8388607 = 2^23 -1;
        ...
        pNleHwInfo->mMaxValue = dMaxSPL;
        *pSPL_Db = 20.0*log10(abs(dMaxSPL)/32767); // 32767 = 2^15 -1;
    }
...
status_t AudioALSANLECtrl::setMaxSPLintoSpecificPos(size_t zuOffsetForTail)
{
    ...
    ALOGVV("mIndexOfChangeValue %d Of %d (Write %d)", mIndexOfChangeValue, mHwBufTotalFrame+mCheckHwExtraFrameCnt, (mLBuf.pWrite-mLBuf.pBufBase)/mBytePerSample);
...
size_t AudioALSANLECtrl::process(void *buffer, size_t Byte)
{
    ...
    if (buffer == NULL || Byte > mStreamAttribute.buffer_size) {
        ALOGE("[Err] NLE process buffer [0x%x] size [%d] (should be less than [%d])", buffer, Byte, mStreamAttribute.buffer_size);
        return 0;
    }
...
status_t AudioALSANLECtrl::getMaxSPLandZeroCnt(NleHwInfo_t* pNleHwInfo __unused, RingBuf* pRingBuf __unused, size_t zuframecount __unused, float* pSPL_Db __unused, size_t* pZeroCnt __unused)
{
    return INVALID_OPERATION;
}
