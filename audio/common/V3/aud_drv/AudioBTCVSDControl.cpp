...
void AudioBTCVSDControl::BT_SCO_TX_Open(void)
{
    ...
    mTXSRCPCMDumpFile = NULL;
    mTXSRCPCMDumpFile = AudioOpendumpPCMFile(mDumpFileName, "CVSDTXOut.pcm.dump");
...
void AudioBTCVSDControl::btsco_process_RX_MSBC(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize, uint8_t packetvalid)
{
    ...
    if ((mBTSCOCVSDContext->pRX->PacketBuf[index1][0] == 0x1) && ((mBTSCOCVSDContext->pRX->PacketBuf[index1][1] & 0xF) == 0x8))
    {
        ...
        status = mMsbcLib.decProcess(mBTSCOCVSDContext->pRX->pDecHandle, (char *)mBTSCOCVSDContext->pRX->EntirePacket, &iInByteTemp, (int16_t *)workbuf, &iOutSample);
...
void AudioBTCVSDControl::BT_SCO_RX_Open(void)
{
    ...
    ALOGD("BT_SCO_RX_Open(+) mBTSCOCVSDContext->uRXState=%d, BTmode=%d", mBTSCOCVSDContext->uRXState, BTmode);
...
void AudioBTCVSDControl::BT_SCO_ExtMDWriteDataToRingBuf(uint8_t *buf, uint32_t size, EXTMD_BTSCO_DIRECTION direction)
{
    ...
    if (size <= (end - pRingBuf->pWrite)) //copy once
    {
...
void AudioBTCVSDControl::BT_SCO_ExtMDReadDataFromRingBuf(uint8_t *buf, uint32_t size, EXTMD_BTSCO_DIRECTION direction)
{
    ...
    if (size <= (end - pRingBuf->pRead)) //copy once
    {
...
void AudioBTCVSDControl::BTCVSD_Init(int mFd2, uint32_t mSourceSampleRate, uint32_t mSourceChannels)
{
    ...
    if (AudioALSALoopbackController::getInstance()->IsAPBTLoopbackWithCodec() == true)
    {
        ...
        mBTCVSDRxTestThread = new AudioBTCVSDLoopbackRxThread(this, 3 , NULL, 0);
    ...
    mAudioBTCVSDControl(AudioBTCVSDControl::getInstance())
{
    ...
    switch (mMemType)
    {
        case 3:
            ...
            mPcm_LoopbackRx= pcm_open(0, 24, PCM_IN, &mConfig_LoopbackRx);
