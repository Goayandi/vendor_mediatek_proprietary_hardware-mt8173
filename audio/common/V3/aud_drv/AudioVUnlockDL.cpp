...
void *ReadRoutine(void *hdl)
{
    ...
    while (!VInstance->mReadThreadExit)
    {
        ...
        if (VInstance->mDLtime.tv_sec == 0 && VInstance->mNewDLtime.tv_sec  != 0)
        {
            ...
            ALOGV("[ReadRoutine] switch to new DL time %d %d", VInstance->mDLtime.tv_sec, VInstance->mDLtime.tv_nsec);
        }
...
int32_t AudioVUnlockDL::SetUplinkStartTime(struct timespec uplinkStartTime, int clear)
{
...
int32_t AudioVUnlockDL:: DoSRC(uint8_t *inbuf, uint32_t *datasz , uint8_t *outbuf, uint32_t *outlength)
{
    ...
    while (count)
    {
        ALOGV("count %d ,in offset %d, datasz %d, out offset %d, outlength %d ", count,inPtr - inbuf, *datasz,outPtr - outbuf, *outlength);
...
int32_t AudioVUnlockDL::GetFirstDLTime()
{
    ...
    if (mGetTime == true)
    {
        ...
        ALOGD("[GetFirstDLTime] DL time  %d,  %d", mNewDLtime.tv_sec, mNewDLtime.tv_nsec);
        mGetTime = false;
        if (mDLtime.tv_sec != 0)
        {
...
int32_t AudioVUnlockDL::SetDownlinkStartTime(int remainMs)
{
    ...
    if (mGetTime == true && mInputStandby == false)
    {
        ...
        ALOGD("[SetDownlinkStartTime] get DL time: mNewDLtime.tv_sec %d, mNewDLtime.tv_nsec %d, remainMs %d", mNewDLtime.tv_sec, mNewDLtime.tv_nsec, remainMs);
        mGetTime = false;
        if (mDLtime.tv_sec != 0)
        {
            mInRemaining = mRingBufIn.GetBufDataSz();
...
int32_t AudioVUnlockDL::ReadRefFromRing(void *buf, uint32_t datasz, void *DLtime)
{
    ...
    ALOGV("[ReadRefFromRing] end finaldataout %d time %d", finaldataout, mDLtime.tv_sec);
...
int32_t  AudioVUnlockDL::GetVoiceUnlockULTime(void *ULtime)
{
    ...
    *ptr = mULtime ;
    ALOGD("ULtime->tv_sec %d, ULtime->tv_nsec %d", ptr->tv_sec, ptr->tv_nsec);
    return 0;
}
