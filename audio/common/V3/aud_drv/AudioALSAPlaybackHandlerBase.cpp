...
status_t AudioALSAPlaybackHandlerBase::initNLEProcessing()
{
    ...
    {
        mNLEMnger = NULL;
        ALOGE("[Err] NLE %s init Fail Line#%d", __FUNCTION__, __LINE__);
    }
    return dRet;
}
...
status_t AudioALSAPlaybackHandlerBase::deinitNLEProcessing()
{
    ...
    {
        ALOGW("[Warn] NLE %s ObjNull Fail Line#%d", __FUNCTION__, __LINE__);
        return NO_INIT;
    }
}
...
status_t AudioALSAPlaybackHandlerBase::doNLEProcessing(void *pInBuffer, size_t inBytes)
{
    ...
    if (mNLEMnger != NULL)
    {
        ...
        if (dRet != NO_ERROR)
        {
            ALOGW("[Warn] NLE %s Line#%d dRet %d", __FUNCTION__, __LINE__, dRet);
        }
        return dRet;
    }
    else
    {
        ALOGE("[Err] NLE %s ObjNull Fail Line#%d", __FUNCTION__, __LINE__);
        return NO_INIT;
    }
}
#else
