...
enum speech_profile_t
{
    ...
    SPEECH_PROFILE_TTY_HANDSET        = 13,
    SPEECH_PROFILE_TTY_HANDSFREE    = 14,

    SPEECH_PROFILE_MAX_NUM      = 15
};
...
//audio type: Speech
typedef struct _SPEECH_CATEGORY_STRUCT
{
    unsigned char NumOfCategoryType;//#define MAX_NUM_CATEGORY_TYPE_SPEECH 4
    unsigned char NumOfMDParam;//#define MAX_NUM_PARAM_SPEECH 3
    unsigned char NumOfMaxParam;//#define MAX_NUM_PARAM_SPEECH 5
    unsigned char NumOfMaxVolume;//#define NUM_VOLUME_SPEECH 7
    unsigned char NumOfMaxNetwork;//#define NUM_NETWORK_SPEECH 1
    unsigned char NumOfMaxBand;//#define NUM_BAND_SPEECH 2

} SPEECH_CATEGORY_STRUCT;
...
class SpeechParamParser
{
    ...
    private:
        ...
        status_t SetMDParamUnitHdr(speech_type_dynamic_param_t idxAudioType, SPEECH_DYNAMIC_PARAM_UNIT_HDR_STRUCT *sParamUnitHdr, uint16_t NumBand);

        uint16_t SetMDParamDataHdr(SPEECH_DYNAMIC_PARAM_UNIT_HDR_STRUCT sParamUnitHdr, Category *cateBand, Category *cateNetwork);
        int InitSpeechNetwork(void);

        SPEECH_PARAM_INFO_STRUCT mSphParamInfo;
        SPEECH_NETWORK_STRUCT ListSpeechNetwork[12];
        uint8_t numSpeechNetwork, mSpeechParamVerFirst, mSpeechParamVerLast;
        bool sphNetworkSupport, sphTTYSupport;
