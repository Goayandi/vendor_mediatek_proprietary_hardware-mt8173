...
static const char PROPERTY_KEY_RIL_PHONE1_MAPPED_MD[PROPERTY_KEY_MAX] = "ril.phone1.mapped.md";
static const char PROPERTY_KEY_RIL_PHONE2_MAPPED_MD[PROPERTY_KEY_MAX] = "ril.phone2.mapped.md";
...
#ifdef MTK_INCALL_MODE_REFACTORY_SUPPORT
    if (bAudioTaste)
    {
        mIdxMD = MODEM_1;
    }
...
#ifndef MTK_INCALL_MODE_REFACTORY_SUPPORT
...
modem_index_t AudioALSASpeechPhoneCallController::updatePhysicalModemIdx(const audio_mode_t audio_mode)
{
    ...
    char PROPERTY_KEY_RIL_PHONE_MAPPED_MD[PROPERTY_KEY_MAX];

    char propertyMDIdx[PROPERTY_VALUE_MAX];
    modem_index_t modem_index = MODEM_1;

    switch (audio_mode)
    {
        case AUDIO_MODE_IN_CALL:
            property_get(PROPERTY_KEY_RIL_PHONE1_MAPPED_MD, propertyMDIdx, "MD1"); //"MD1": default Modem
            strcpy(PROPERTY_KEY_RIL_PHONE_MAPPED_MD, PROPERTY_KEY_RIL_PHONE1_MAPPED_MD);
            break;

        case AUDIO_MODE_IN_CALL_2:
            property_get(PROPERTY_KEY_RIL_PHONE2_MAPPED_MD, propertyMDIdx, "MD1"); //"MD1": default Modem
            strcpy(PROPERTY_KEY_RIL_PHONE_MAPPED_MD, PROPERTY_KEY_RIL_PHONE2_MAPPED_MD);
            break;

        default:
            ALOGE("%s(), Invalid audio_mode %d, ASSERT!!!", __FUNCTION__, audio_mode);
            ASSERT(0);
            break;
    }
    ALOGD("%s(), %s = %s", __FUNCTION__, PROPERTY_KEY_RIL_PHONE_MAPPED_MD, propertyMDIdx);

    if (strcmp(propertyMDIdx, "MD1") == 0)
    {
        modem_index = MODEM_1;
        ALOGD("%s(), %s = %s set MD1 modem_index=%d", __FUNCTION__, PROPERTY_KEY_RIL_PHONE_MAPPED_MD, propertyMDIdx, modem_index);
    }
    else if (strcmp(propertyMDIdx, "MD2") == 0)
    {
        modem_index = MODEM_2;
        ALOGD("%s(), %s = %s set MD2 modem_index=%d", __FUNCTION__, PROPERTY_KEY_RIL_PHONE_MAPPED_MD, propertyMDIdx, modem_index);
    }
    else if (strcmp(propertyMDIdx, "MD3") == 0)
    {
        modem_index = MODEM_EXTERNAL;
        ALOGD("%s(), %s = %s set MD3 modem_index=%d", __FUNCTION__, PROPERTY_KEY_RIL_PHONE_MAPPED_MD, propertyMDIdx, modem_index);
    }
    else
    {
        ALOGE("%s(), %s = %s, Invalid MD Index, ASSERT!!!!!!", __FUNCTION__, PROPERTY_KEY_RIL_PHONE_MAPPED_MD, propertyMDIdx);
        ASSERT(0);
    }
    ALOGD("-%s(), %s = %s, modem_index=%d", __FUNCTION__, PROPERTY_KEY_RIL_PHONE_MAPPED_MD, propertyMDIdx, modem_index);

    return modem_index;
#else
    ALOGW("%s() unsupport! return default MD1, audio_mode %d", __FUNCTION__, audio_mode);
    return MODEM_1;
#endif
}
