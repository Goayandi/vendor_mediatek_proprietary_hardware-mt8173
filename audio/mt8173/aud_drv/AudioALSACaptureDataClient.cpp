...
#define BOOST_ECHOREF

#ifndef DROP_MS_AFTER_BESRECORD_PROCESS
#define DROP_MS_AFTER_BESRECORD_PROCESS 0
#endif
//BesRecord---
...
static const uint32_t kClientBufferSize = 0x20000; // 128k
#else
    ...
    //echoref---
{
    ...
    ALOGV("%s(), besrecord_enable=%d, besrecord_scene=%d", __FUNCTION__,
    ...
    if (mStreamAttributeTarget->BesRecord_Info.besrecord_enable)
    {
        ...
        ALOGD("sample rate = %u, drop ms = %d, channels = %u, byts per sample = %zd, dropBesRecordDataSize = %u\n",
    ...
    {
        ALOGV("%s sample_rate: %d => %d, num_channels: %d => %d, audio_format: 0x%x => 0x%x",
              __FUNCTION__, mStreamAttributeSource->sample_rate, mStreamAttributeTarget->sample_rate,
...
AudioALSACaptureDataClient::~AudioALSACaptureDataClient()
{
    ...
    {
        ALOGV("%s(), remove EchoRef data provider,mCaptureDataProviderEchoRef=%p", __FUNCTION__, mCaptureDataProviderEchoRef);
...
{
    ALOGV("%s(), mIsIdentitySet=%d, identity=%d", __FUNCTION__, mIsIdentitySet, identity);
    if (!mIsIdentitySet)
    {
        mIdentity = identity;
        mIsIdentitySet = true;
...
uint32_t AudioALSACaptureDataClient::copyCaptureDataToClient(RingBuf pcm_read_buf)
{
    ...
    else    //no need to do BesRecord PreProcess, transform data to mStreamAttributeTarget format
    {
        ...
        else // Need SRC
        {
            ...
            ALOGV("%s(), pRead:%" PRIdPTR ", pWrite:%" PRIdPTR ", dataCount:%d", __FUNCTION__,
                  mSrcDataBuf.pRead - mSrcDataBuf.pBufBase,
                  mSrcDataBuf.pWrite - mSrcDataBuf.pBufBase,
...
ssize_t AudioALSACaptureDataClient::read(void *buffer, ssize_t bytes)
{
    ...
    ALOGV("-%s() ReadDataBytes = %d Queued = %d",
          __FUNCTION__, ReadDataBytes, RingBuf_getDataCount(&mProcessedDataBuf));
...
uint32_t AudioALSACaptureDataClient::TransferFormat(char * linear_buffer ,audio_format_t src_format ,audio_format_t des_format, uint32_t bytes)
{
    ...
    if (des_format == AUDIO_FORMAT_PCM_24_BIT_PACKED) //convert 8+24 to 24 bit
    {
        ...
        {
            for (int32_t i = 1; i < (int32_t)(bytes / src_bit); i++)
            {
    ...
    if (des_format == AUDIO_FORMAT_PCM_16_BIT) //convert 8+24 to 16 bit
    {
        ...
        if (src_format == AUDIO_FORMAT_PCM_8_24_BIT)
        {
            for (int32_t i = 0; i < (int32_t)(bytes / src_bit); i++)
            {
...
status_t AudioALSACaptureDataClient::start()
{
    if (mCaptureDataProvider)
        mCaptureDataProvider->start(this);

    if (mCaptureDataProviderEchoRef)
        mCaptureDataProviderEchoRef->start(this);
...
void AudioALSACaptureDataClient::CheckChannelRemixOp(void)
{
    ...
    ALOGV("%s(), ch %d->%d (mBesRecordStereoMode = %d, BesRecord enable = %d), ChannelRemixOp = %d",
        __FUNCTION__,
        sourceChannel,
        targetChannel,
...
ssize_t AudioALSACaptureDataClient::ApplyChannelRemixWithRingBuf(RingBuf *srcBuffer, RingBuf *dstBuffer)
{
    ...
    if (!dataSizeAfterProcess || !dataSize)
        return 0;
...
void AudioALSACaptureDataClient::LoadBesRecordParams(void)
{
    ...
    for (int i = 0; i < total_num_scenes; i++)
        ...
        {
            ALOGV("scene_table[%d][%d] = %d", i, j, mBesRecordSceneTable.scene_table[i][j]);
        }
#endif
    ALOGV("-%s()", __FUNCTION__);
#endif
...
int AudioALSACaptureDataClient::CheckBesRecordMode(void)
{
    ...
    {
        ALOGV("%s, is BT device", __FUNCTION__);
        if (mBesRecordSceneTable.scene_table[NORMAL_RECORDING_DEFAULT_MODE][HD_REC_DEVICE_SOURCE_BT_EARPHONE] != 0xFF)
        {
            ...
            ALOGV("%s, get specific BT modeIndex = %d", __FUNCTION__, modeIndex);
        }
        else
        {
    ...
    else if ((u4SceneIdx >= 0) && (u4SceneIdx < MAX_HD_REC_SCENES))
    {
            ...
            && (mStreamAttributeTarget->input_device == AUDIO_DEVICE_IN_WIRED_HEADSET))
        {
            ...
            ALOGV("%s, is HEADSET device, u4SceneIdx=%d, modeIndex=%d", __FUNCTION__, u4SceneIdx, modeIndex);
        }
        // Handset Mic
        else if (mBesRecordSceneTable.scene_table[u4SceneIdx][HD_REC_DEVICE_SOURCE_HANDSET] != 0xFF)
        {
            ...
            ALOGV("%s, is HANDSET device, u4SceneIdx=%d, modeIndex=%d", __FUNCTION__, u4SceneIdx, modeIndex);
        ...
        {
            ALOGV("%s, Handset mode index shoule not be -1, u4SceneIdx=%d, modeIndex=%d", __FUNCTION__, u4SceneIdx, modeIndex);
        }
#ifdef UPLINK_LOW_LATENCY
...
#endif
        // Debug print
        ALOGV("GetBesRecordModeInfo: map_fir_ch1=%d, map_fir_ch2=%d",
    ...
    if (RecordModeGet == false)
    {
        ...
        ALOGV("%s, use default mode input_device=%x, mBesRecordSceneIndex = %d ", __FUNCTION__, mStreamAttributeTarget->input_device, mBesRecordSceneIndex);
    ...
    ALOGV("-%s(), mBesRecordSceneIndex=%d, modeIndex=%d", __FUNCTION__, mBesRecordSceneIndex, modeIndex);
...
void AudioALSACaptureDataClient::ConfigBesRecordParams(void)
{
    ...
    ALOGV("%s(),mBesRecordStereoMode=%d, input_source= %d, input_devices=%x,mBesRecordModeIndex=%d, bVoIPEnable=%d, mode=%d, bypassDualProcess=%d", __FUNCTION__, mBesRecordStereoMode, mStreamAttributeTarget->input_source,
    ...
    for (int i = 0; i < EnhanceParasNum; i++) //EnhanceParasNum = (16+32)+12(common parameters)
    {
        ...
        ALOGV("BesRecordEnhanceParas[%d]=%u", i, BesRecordEnhanceParas[i]);
    }
    ...
    if (mStreamAttributeTarget->input_device == AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET)
    {
        gain = 0;
        TotalGain = 0;
        ALOGV("BT path set Digital MIC gain = 0");
    }
...
uint32_t AudioALSACaptureDataClient::BesRecordPreprocess(void *buffer , uint32_t bytes)
{
    ...
    memset(&InBufinfo, 0, sizeof(struct InBufferInfo));
...
{
    int RoutePath;
    ALOGV("+%s(), %x", __FUNCTION__, mStreamAttributeTarget->output_devices);
...
{
    ALOGV("%s() %d", __FUNCTION__, mBypassBesRecord);
...
bool AudioALSACaptureDataClient::CheckNeedBesRecordSRC()
{
    ...
    if (mStreamAttributeTarget->BesRecord_Info.besrecord_enable == true)
    {
        ...
        ALOGV("%s(), mStreamAttributeSource->sample_rate=%d, mStreamAttributeSource->num_channels=%d, mStreamAttributeTarget->sample_rate=%d,mStreamAttributeTarget->num_channels=%d, BesRecord_usingsamplerate=%d"
    ...
    ALOGV("%s(), %d, %d, mBesRecSRCSizeFactor=%d", __FUNCTION__, mNeedBesRecordSRC, BesRecord_usingsamplerate, mBesRecSRCSizeFactor);
...
{
    ALOGV("+%s() besrecord_voip_enable %d, besrecord_enable=%d", __FUNCTION__,
...
void AudioALSACaptureDataClient::UpdateDynamicFunction(void)
{
    ...
    ALOGV("%s(), RoutePath %d, mode %d", __FUNCTION__, RoutePath, mode);
...
{
    ALOGV("%s(), type=%d, bypassDMNR=%d", __FUNCTION__, type,mStreamAttributeTarget->BesRecord_Info.besrecord_bypass_dualmicprocess);
    ...
    if (((QueryFeatureSupportInfo()& SUPPORT_DUAL_MIC) > 0) && (mStreamAttributeTarget->BesRecord_Info.besrecord_bypass_dualmicprocess == false))
    {
        ...
        else
        {
            ...
            ALOGI("%s(), turn off due to not support", __FUNCTION__);
            mSPELayer->SetDynamicFuncCtrl(NORMAL_DMNR, false);
            mSPELayer->SetDynamicFuncCtrl(HANDSFREE_DMNR, false);
        }
    }
...
void AudioALSACaptureDataClient::CheckNativeEffect(void)
{
    ...
    {
        ALOGV("+%s() %d", __FUNCTION__, mStreamAttributeTarget->NativePreprocess_Info.PreProcessEffect_Count);
...
void AudioALSACaptureDataClient::AddEchoRefDataProvider(AudioALSACaptureDataProviderBase *pCaptureDataProvider, stream_attribute_t *stream_attribute_target)
{
...
#ifdef MTK_HIRES_192K_AUDIO_SUPPORT
    if (mStreamAttributeTargetEchoRef->sample_rate < 16000)
        mStreamAttributeTargetEchoRef->sample_rate = 16000;
#endif
    ...
    ALOGV("%s(), mCaptureDataProviderEchoRef=%p", __FUNCTION__, mCaptureDataProviderEchoRef);
    mCaptureDataProviderEchoRef->attach(this); // mStreamAttributeSource will be updated when first client attached


    ALOGV("%s(), Source sample_rate=%d, num_channels=%d, audio_format=%d", __FUNCTION__
    ...
    ALOGV("%s(), Target sample_rate=%d, num_channels=%d, audio_format=%d", __FUNCTION__
