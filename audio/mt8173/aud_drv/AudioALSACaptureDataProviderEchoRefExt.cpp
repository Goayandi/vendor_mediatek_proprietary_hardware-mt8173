...
status_t AudioALSACaptureDataProviderEchoRefExt::open()
{
    ...
    unsigned int pcmindex = AudioALSADeviceParser::getInstance()->GetPcmIndexByString(keyCustomPcmI2S0AwbCapture);
    unsigned int cardindex = AudioALSADeviceParser::getInstance()->GetCardIndexByString(keyCustomPcmI2S0AwbCapture);
