...
uint32_t MapVoiceVolumetoCustom(uint32_t mode, uint32_t gain)
{
    ...
    ALOGD("MapVoiceVolumetoCustom - gain:%d, mappingGain:%d", gain, mappingGain);
...
static void *Play_PCM_With_SpeechEnhance_Routine(void *arg)
{
    ...
    ALOGD("Play_PCM_With_SpeechEnhance_Routine in +");
    AudioALSAParamTuner *pAUDParamTuning = (AudioALSAParamTuner *)arg;

    if (pAUDParamTuning == NULL)
    {
        ALOGE("Play_PCM_With_SpeechEnhance_Routine pAUDParamTuning = NULL arg = %x", arg);
        return 0;
    }
    ...
    ALOGD("pthread_cond_signal(&pAUDParamTuning->mPPSExit_Cond), buffer size=%d", PCM_BUF_SIZE);
    ...
    while ((!pAUDParamTuning->m_bPPSThreadExit) && pFd)
    {
        ...
        ALOGV(" Playback buffer, free:%d, read from :%d, total play:%d", playBufFreeCnt, cntR, numOfBytesPlayed);
        pthread_mutex_unlock(&pAUDParamTuning->mPlayBufMutex);

        if (cntR < playBufFreeCnt)
        {
            ALOGD("File reach the end");
    ...
    //exit thread
    ALOGD("playbackRoutine pthread_mutex_lock");
    pthread_mutex_lock(&pAUDParamTuning->mPPSMutex);
    ALOGD("pthread_cond_signal(&pAUDParamTuning->mPPSExit_Cond)");
    pthread_cond_signal(&pAUDParamTuning->mPPSExit_Cond); // wake all thread
    pthread_mutex_unlock(&pAUDParamTuning->mPPSMutex);
    return 0;
}
...
{
    ALOGD("DMNR_Play_Rec_Routine in +");
    AudioALSAParamTuner *pDMNRTuning = (AudioALSAParamTuner *)arg;
    if (pDMNRTuning == NULL)
    {
        ALOGE("DMNR_Play_Rec_Routine pDMNRTuning = NULL arg = %x", arg);
        return 0;
    }
    ...
    ALOGD("pthread_cond_signal(&pDMNRTuning->mDMNRExit_Cond)");
    ...
    while ((!pDMNRTuning->m_bDMNRThreadExit) && pOutFp)
    {
        ...
        if (pInFp)
        {
            ...
            numOfBytesPlay += cntR;
            ALOGV(" Playback buffer, free:%d, read from :%d, total play:%d", playBufFreeCnt, cntR, numOfBytesPlay);
        }
        ...
        numOfBytesRec += cntW;
        ALOGV(" Record buffer, available:%d, write to file:%d, total rec:%d", recBufDataCnt, cntW, numOfBytesRec);
    ...
    pthread_mutex_lock(&pDMNRTuning->mDMNRMutex);
    ALOGD("pthread_cond_signal(&mDMNRExit_Cond)");
    pthread_cond_signal(&pDMNRTuning->mDMNRExit_Cond); // wake all thread
    pthread_mutex_unlock(&pDMNRTuning->mDMNRMutex);
...
{
    ALOGD("DMNR_Play_Rec_ApSide_Routine in +");
    AudioALSAParamTuner *pDMNRTuning = (AudioALSAParamTuner *)arg;
    if (pDMNRTuning == NULL)
    {
        ALOGE("DMNR_Play_Rec_ApSide_Routine pDMNRTuning = NULL arg = %p", arg);
        return 0;
    }
    ...
    if (pInFp) //open output stream for playback
    {
        ...
        {
            ALOGE("[Dual-Mic] playback file format is not support");
            return 0;
        }
        ...
        ALOGD("readBlockLen = %d, hwBufferSize = %d,playbackDb_index = %d \n", readBlockLen, hwBufferSize, playbackDb_index);
    }
    ...
    while (!pDMNRTuning->m_bDMNRThreadExit)
    {
        ...
        if (pInFp && !feof(pInFp))
        {
            ...
            memset(inBuffer, 0, sizeof(char)*readBlockLen);
            memset(outBuffer, 0, sizeof(char)*hwBufferSize);
...
AudioALSAParamTuner *AudioALSAParamTuner::getInstance()
{
    ...
    {
        ALOGD("create AudioParamTuning instance --");
        UniqueTuningInstance = new AudioALSAParamTuner();
        ALOGD("create AudioALSAParamTuner instance ++");
    }
    ...
    m_bPlaying(false)
{
    ...
    if (ret != 0)
    {
        ALOGE("Failed to initialize pthread mP2WMutex!");
    }

    ret = pthread_mutex_init(&mPPSMutex, NULL);
    if (ret != 0)
    {
        ALOGE("Failed to initialize mPPSMutex!");
    }

    ret = pthread_mutex_init(&mPlayBufMutex, NULL);
    if (ret != 0)
    {
        ALOGE("Failed to initialize mPlayBufMutex!");
    }

    ret = pthread_cond_init(&mPPSExit_Cond, NULL);
    if (ret != 0)
    {
        ALOGE("Failed to initialize mPPSExit_Cond!");
    }
    ...
    if (ret != 0)
    {
        ALOGE("Failed to initialize mDMNRMutex!");
    }

    ret = pthread_mutex_init(&mRecBufMutex, NULL);
    if (ret != 0)
    {
        ALOGE("Failed to initialize mRecBufMutex!");
    }

    ret = pthread_cond_init(&mDMNRExit_Cond, NULL);
    if (ret != 0)
    {
        ALOGE("Failed to initialize mDMNRExit_Cond!");
    }

    ALOGD("AudioALSAParamTuner: default mic gain-mormal:%d;handsfree:%d, receiver gain:%d, headset Gain:%d", mDualMicTool_micGain[0], mDualMicTool_micGain[1], mDualMicTool_receiverGain,
...
{
    ALOGV("isPlaying - playing:%d", m_bPlaying);
    bool ret = false;
    pthread_mutex_lock(&mP2WMutex);
#if defined(MTK_DUAL_MIC_SUPPORT) || defined(MTK_AUDIO_HD_REC_SUPPORT)
    ALOGV("isPlaying - DMNR playing:%d", m_bDMNRPlaying);
...
status_t AudioALSAParamTuner::setDLPGA(uint32_t gain)
{
    ...
    {
        ALOGE("setDLPGA gain error  gain=%x", gain);
        return BAD_VALUE;
    }

    pthread_mutex_lock(&mP2WMutex);
    mOutputVolume[mMode] = gain;
    ALOGD("setDLPGA mode=%d, gain=%d, lad volume=0x%x", mMode, gain, mOutputVolume[mMode]);

    if (m_bPlaying)
    {
        ALOGD("setDLPGA lad_Volume=%x", mOutputVolume[mMode]);
...
{
#ifdef MTK_AUDIO_PHONE_CALL_SUPPORT
    ALOGD("updataOutputFIRCoffes in +");
    ...
    if (pCustParam->wb_mode)
    {
        ...
        {
            ALOGD("updataOutputFIRCoffes mode=%d, ori dlDG gain=%d, new dlDG gain=%d", mode, sCustWbParam.speech_mode_wb_para[mode][7], pCustParam->dlDigitalGain);
    ...
    else
    {
        ...
        {
            ALOGD("updataOutputFIRCoffes mode=%d, old index=%d, new index=%d", mode, sCustMedParam.select_FIR_output_index[mode], index);
            //save  index to MED with different mode.
            sCustMedParam.select_FIR_output_index[mode] = index;

            ALOGD("updataOutputFIRCoffes ori sph_out_fir[%d][0]=%d, ori sph_out_fir[%d][44]", mode, sCustParam.sph_out_fir[mode][0], mode, sCustParam.sph_out_fir[mode][44]);
            ...
            ALOGD("updataOutputFIRCoffes new sph_out_fir[%d][0]=%d, new sph_out_fir[%d][44]", mode, sCustParam.sph_out_fir[mode][0], mode, sCustParam.sph_out_fir[mode][44]);
        ...
        {
            ALOGD("updataOutputFIRCoffes mode=%d, old dlDGGain=%d, new dlDGGain=%d", mode, sCustParam.speech_mode_para[mode][7], dlGain);
            sCustParam.speech_mode_para[mode][7] = dlGain;
        }
        ALOGD("updataOutputFIRCoffes  sph_out_fir[%d][0]=%d, sph_out_fir[%d][44]", mode, sCustParam.sph_out_fir[mode][0], mode, sCustParam.sph_out_fir[mode][44]);
...
{
#ifdef MTK_AUDIO_PHONE_CALL_SUPPORT
    ALOGD("enableModemPlaybackVIASPHPROC bEnable:%d, bWBMode:%d", bEnable, bWB);
    int ret = 0;
    ...
    if (bEnable && (isPlaying() == false))
    {
        ...
        mSphPhonecallCtrl->open((audio_mode_t)mMode, mAudioResourceManager->getOutputDevice(), mAudioResourceManager->getInputDevice());
    ...
    {
        ALOGD("The Audio Taste Tool State is error, bEnable=%d, playing=%d", bEnable, m_bPlaying);
        return BAD_VALUE;
    }
#else
    (void)bEnable;
...
{
    ALOGD("playbackFileFormat - playback file name:%s", m_strInputFileName);
    ...
    if (pFileSuffix != NULL)
    {
        ...
        {
            ALOGD("playbackFileFormat - playback file format is pcm");
            ret = PCM_FORMAT;
        }
        else if (strcmp(pFileSuffix, "wav") == 0 || strcmp(pFileSuffix, "WAV") == 0)
        {
            ALOGD("playbackFileFormat - playback file format is wav");
            ret = WAVE_FORMAT;
        }
        else
        {
            ALOGD("playbackFileFormat - playback file format is unsupport");
...
{
    ALOGD("setDMNRGain: type=%d, gain=%d", type, value);
    ...
    switch (type)
    {
        ...
        case AUD_MIC_GAIN_HF:
            ...
            break;
        default:
            ALOGE("setDMNRGain unknown type");
            ret = BAD_VALUE;
            break;
    }
    pthread_mutex_unlock(&mP2WMutex);
    ...
    switch (type)
    {
        ...
        case AUD_MIC_GAIN_HF:
            *value = mDualMicTool_micGain[1]; //handsfree mic
            break;
        default:
            ALOGE("getDMNRGain unknown type");
            ret = BAD_VALUE;
            break;
    }
    pthread_mutex_unlock(&mP2WMutex);
...
{
    ALOGV("setPlaybackVolume in +");
    ALOGD("gain:%u, mode:%u, device=%u", gain, mode, device);
    ...
    ALOGV("setPlaybackVolume in -, mPlaybackDb_index=%d", mPlaybackDb_index);
    return NO_ERROR;
}
...
{
    ALOGD("enableDMNRAtApSide bEnable:%d, wb mode:%d, outputDevice:%d,work mode:%d", bEnable, bWBMode, outputDevice, workMode);
    ...
    if (bEnable && (isPlaying() == false))
    {
        ...
        {
            m_pInputFile = fopen(m_strInputFileName, "rb");
            ALOGD("[Dual-Mic] open input file filename:%s", m_strInputFileName);
            if (m_pInputFile == NULL)
            {
                ALOGE("[Dual-Mic] open input file fail!!");
                pthread_mutex_unlock(&mP2WMutex);
                return BAD_VALUE;
            }
        }
        ...
        ALOGD("[Dual-Mic] open output file filename:%s", m_strOutFileName);
        if (m_pOutputFile == NULL)
        {
            ALOGE("[Dual-Mic] open output file fail!!");
            fclose(m_pInputFile);
            pthread_mutex_unlock(&mP2WMutex);
            return BAD_VALUE;
        }
        ...
        if ((workMode == RECPLAY_MODE) || (workMode == RECPLAY_HF_MODE))
        {
            ...
            ALOGD("Play+Rec set dual mic at ap side, dev:0x%x, mode:%d, gain:%d", dev, mode, volume);
        }

        // open buffer thread
        ALOGD("open DMNR Tuning At Ap side threadloop thread~");
        pthread_mutex_lock(&mDMNRMutex);
        ret = pthread_create(&mDMNRThreadID, NULL, DMNR_Play_Rec_ApSide_Routine, (void *)this);
        if (ret != 0)
        {
            ALOGE("DMNR Tuning At Ap side pthread_create error!!");
        }

        ALOGD("+mDMNRExit_Cond wait");
        ret = pthread_cond_timedwait(&mDMNRExit_Cond, &mDMNRMutex, &timeout);
        ALOGD("-mDMNRExit_Cond receive ret=%d", ret);
    ...
    else if (!bEnable && m_bDMNRPlaying)
    {
        pthread_mutex_lock(&mP2WMutex);
        //stop buffer thread
        ALOGD("close DMNR Tuning At Ap side");
        pthread_mutex_lock(&mDMNRMutex);
    ...
    {
        ALOGD("The DMNR Tuning State is error, bEnable=%d, playing=%d", bEnable, m_bPlaying);
        return BAD_VALUE;
    }
