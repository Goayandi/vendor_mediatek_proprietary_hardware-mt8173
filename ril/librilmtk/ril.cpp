...
static const struct timeval TIMEVAL_WAKE_TIMEOUT_MTK = {1,0};
...
const int s_mtk_commands_size = 0;
...
#endif
{
    ...
    if (fdCommand == -1) {
        RLOGD("Can't send URC because there is no connection yet. Try to cache request:%s in RIL%s", requestToString(unsolResponse), rilSocketIdToString(soc_id));
