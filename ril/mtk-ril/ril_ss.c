...
convertToUCS2(char* data)
{
    ...
    LOGW("convertToUCS2= ", ucs2HexString);

    return ucs2HexString;
}
...
static void onUssd(char *s, RIL_SOCKET_ID rid)
{
    ...
    if (at_tok_hasmore(&line)) {
        ...
        LOGD("onUssd: p_data[0] old value = %s", p_data[0]);
        if (strcmp(p_data[0],"2") == 0 || strcmp(p_data[0],"3") == 0 ||
            strcmp(p_data[0],"4") == 0 || strcmp(p_data[0],"5") == 0) {
            mappingString = strdup("0");
            p_data[0] = mappingString;
            LOGD("onUssd: p_data[0] new value = %s", p_data[0]);
        }
    ...
    size_t   responselen = length * sizeof(char *);
    char**   p_cur = (char **) p_data;
    ...
    for (i = 0 ; i < numStrings ; i++) {
        if (i == 1) {
            ...
            finalData[1] = (const char *) utf8String;
