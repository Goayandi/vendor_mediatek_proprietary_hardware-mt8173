...
int getSingnalStrength(char *line, int *response)
{
        ...
        *rsrp_in_qdbm = INVALID;
        ...
        if ((*act == 0) || (*act == 1) || (*act == 3)){  // for GSM: <rxlevel>,<ber>
            ...
            *rsrp_in_qdbm = INVALID;
            ...
            LOGD("Act==%d, force ECSQ <sig1>:%d -> 0", *act, *sig1);
            *sig1 = 0;
        } else {
    ...
    // For 3G network, we'll reset rssi value to meet GSM signal level
    if (is_Lte != 1 && (*rscp_in_qdbm) < 0) {
        if ((*sig1) == 99) {
            (*sig1) = 99;
        } else if ((*sig1) >= 15) {
            (*sig1) = 12;
        ...
        LOGD("Reset rssi value to meet GSM signal level: %d", (*sig1));
    }
...
//            if (response[3] > 6)
    ...
    if (is_cs && response[0] != 1 && response[0] != 5) {
        int state = 0;
        if (bEmergencyOnly[rid]) {
            if (response[0] == 255)
                response[0] = 2;
            state = response[0]+10;
        } else {
    ...
    else if (response[0] != 2 && response[0] != 255) // if 2 or 255, it is searching, no need to set restricted state
    {
        // LOGE("!!!!out service");
        if (is_cs)
        {
...
void requestQueryAvailableNetworks(void * data, size_t datalen, RIL_Token t)
{
    ...
    // TODO: remove access technology information to comply original RIL responseOperatorInfos
    response = (char **) malloc( sizeof(char*) * num *4); // for string, each one is 20 bytes
    block_p = (char *) malloc(num* sizeof(char)*4*MAX_OPER_NAME_LENGTH);
...
// TODO: remove access technology information to comply original RIL responseOperatorInfos
    ...
    // filter the response
    response_filter = (char**)malloc(sizeof(char*) * num_filter * 4);
...
void requestSetPreferredNetworkType(void * data, size_t datalen, RIL_Token t)
{
    ...
    ****************************************/
    ...
    if(rat >= 0)
    {
        err = at_send_command("AT+EGTP=1", NULL, getRILChannelCtxFromToken(t));
        err = at_send_command("AT+EMPPCH=1", NULL, getRILChannelCtxFromToken(t));
...
static int getCellInfoListV12(char * line, int num, RIL_CellInfo_v12 * response)
{
    ...
    */
    ...
    int ext3_ext4_enabled = 1; //set to 1 if modem support
...
void onNetworkStateChanged(char *urc, const RIL_SOCKET_ID rid)
{
    ...
    */
    if (is_cs == 1)
    {
        // check <lac> and <ci> to see if currently there is no service
        LOGD("onNetworkStateChanged(): %d, %d, %d, %d, %d", stat, response[0], response[1], response[2], response[3]);
        if ((response[0] == 0 || response[0] == 0xffff) && (response[1] == 0 || response[1] == 0xffff))
            bEmergencyOnly[rid] = 0;
        else
            bEmergencyOnly[rid] = 1;
...
void onRestrictedStateChanged(const RIL_SOCKET_ID rid)
{
    ...
    if ((rid == RIL_SOCKET_1 && (sim_inserted_status & SIM1_INSERTED))
#ifdef MTK_GEMINI
        || (rid == RIL_SOCKET_2 && (sim_inserted_status & SIM2_INSERTED))
#if (SIM_COUNT >= 3)
        || (rid == RIL_SOCKET_3 && (sim_inserted_status & SIM3_INSERTED))
#endif
#if (SIM_COUNT >= 4)
        || (rid == RIL_SOCKET_4 && (sim_inserted_status & SIM4_INSERTED))
#endif
#endif
        )
    {
...
void onCellInfoList(char *urc, const RIL_SOCKET_ID rid)
{
    ...
    err = getCellInfoListV12(urc, num, response);
    if (err < 0) goto error;
