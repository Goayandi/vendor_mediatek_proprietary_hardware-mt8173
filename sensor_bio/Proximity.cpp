      ...
      mSensorReader(32)
{
    ...
    mdata_fd = FindDataFd();
    if (mdata_fd >= 0) {
        strcpy(input_sysfs_path, "/sys/class/sensor/m_ps_misc/");
...
int ProximitySensor::flush(int handle)
{
    ...
    ALOGD("ps flush\n");
    strcpy(&input_sysfs_path[input_sysfs_path_len], "psflush");
    ALOGD("ps flush path:%s \r\n",input_sysfs_path);
    fd = open(input_sysfs_path, O_RDWR);
    if(fd < 0) {
        ALOGD("no ps flush control attr\r\n");
        return -1;
    }
