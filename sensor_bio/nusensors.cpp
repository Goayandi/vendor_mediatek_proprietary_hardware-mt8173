    ...
    do {
        ...
        for (int i=0 ; count && i<numFds ; i++) {
            ...
            if (data->type == SENSOR_TYPE_SIGNIFICANT_MOTION) {
                ...
                data += nb;
                //if(nb < 0||nb > count)
                //ALOGE("sensor: %d, pollEvents count nb:%d, count:%d, nbEvents:%d",i, nb, count, nbEvents);//for sensor NE debug
            }
        }

        //ALOGE("pollEvents count 1:%d", count);//for sensor NE debug
        if (count) {
            // we still have some room, so try to see if we can get
            // some events immediately or just wait if we don't have
            // anything to return
            //ALOGE("pollEvents count 2:%d", count);//for sensor NE debug
