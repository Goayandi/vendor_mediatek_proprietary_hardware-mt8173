      ...
      mSensorReader(32)
{
    ...
    mdata_fd = FindDataFd();
    if (mdata_fd >= 0) {
        strcpy(input_sysfs_path, "/sys/class/sensor/m_baro_misc/");
...
int PressureSensor::batch(int handle, int flags, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    ...
    ALOGD("pressure batch: handle:%d, flag:%d,samplingPeriodNs:%lld maxBatchReportLatencyNs:%lld\r\n",
        handle, flags,samplingPeriodNs, maxBatchReportLatencyNs);

    strcpy(&input_sysfs_path[input_sysfs_path_len], "barobatch");
    ALOGD("pressure batch path:%s \r\n",input_sysfs_path);
    fd = open(input_sysfs_path, O_RDWR);
    if(fd < 0) {
        ALOGD("no pressure batch control attr\r\n");
        return -1;
    }
...
int PressureSensor::flush(int handle)
{
    ...
    ALOGD("pressure flush\n");
    strcpy(&input_sysfs_path[input_sysfs_path_len], "baroflush");
    ALOGD("pressure flush path:%s \r\n",input_sysfs_path);
    fd = open(input_sysfs_path, O_RDWR);
    if(fd < 0) {
        ALOGD("no pressure flush control attr\r\n");
        return -1;
    }
