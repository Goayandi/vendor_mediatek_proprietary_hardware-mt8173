...
int format_root_device(const char *root)
{
    ...
    int result = ensure_root_path_unmounted(root);
    if (result < 0) {
	...
	struct selabel_handle *sehnd = selabel_open(SELABEL_CTX_FILE, seopts, 1);
		...
		rec = fs_mgr_get_entry_for_mount_point(fstab, "/data");
		if (!rec) {
			LOGE("failed to mount userdata\n");
			ret = -1;
			goto out_without_free;
		}

		asprintf(&source, "%s", rec->blk_device);
		int result = make_ext4fs(source, 0, root, sehnd);
...
#ifdef MTK_GPT_SCHEME_SUPPORT
		rec = fs_mgr_get_entry_for_mount_point(fstab, "/cache");
		if (!rec) {
			LOGE("failed to mount cache\n");
			ret = -1;
			goto out_without_free;
		}

		asprintf(&source, "%s", rec->blk_device);
		int result = make_ext4fs(source, 0, 0, 0);
...
#ifdef MTK_GPT_SCHEME_SUPPORT
		rec = fs_mgr_get_entry_for_mount_point(fstab, "/nvdata");
		if (!rec) {
			LOGE("failed to mount nvdata\n");
			ret = -1;
			goto out_without_free;
		}
