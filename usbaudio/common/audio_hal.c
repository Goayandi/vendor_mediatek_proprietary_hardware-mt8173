...
#ifdef MTK_AUDIO_DEBUG
    ALOGD(USB_TAG"out_get_buffer_size %d, period size=%d", buffer_size,proxy_get_period_size(&out->proxy));
#endif
    return buffer_size;
}
...
static ssize_t out_write(struct audio_stream_out *stream, const void* buffer, size_t bytes)
{
    ...
    if (out->standby) {
        pthread_mutex_lock(&out->dev->lock);
        ALOGD(USB_TAG"+out_write(%d), start_output_stream",bytes);
    ...
    if (write_buff != NULL && num_write_buff_bytes != 0) {
        ...
        mOldtime = mNewtime;
        //Dump debug data
        dumpPcmData(usbstreamout,(void*)write_buff,num_write_buff_bytes,streamout_propty);
    }
    else {
                                   ...
                                   const char *address /*__unused*/)
{
...
#ifdef MTK_AUDIO_DEBUG
    ALOGD("usb:audio_hal::out adev_open_output_stream() handle:0x%X, device:0x%X, flags:0x%X",
    ...
    ALOGD(USB_TAG"adev_open_output_stream, proxy_config.rate=%d, proxy_config.format=%d, channel=%d",proxy_config.rate,proxy_config.format,proposed_channel_count);
...
#ifdef MTK_AUDIO_DEBUG
    ALOGD("usb:audio_hal::start_input_stream(card:%d device:%d)",
...
static ssize_t in_read(struct audio_stream_in *stream, void* buffer, size_t bytes)
{
    ...
    if (in->standby) {
        ...
        pthread_mutex_unlock(&in->dev->lock);
        if (ret != 0) {
        ALOGD(USB_TAG"+in_read(%d), start_input_stream",bytes);
            goto err;
        }
        in->standby = false;
    }
...
err:
    ...
    ALOGV("audio_hal:usb in_read, bytes =%d, num_read_buff_bytes=%d", bytes,num_read_buff_bytes);
    //Dump debug data
    dumpPcmData(usbstreamin,(void*)buffer,num_read_buff_bytes,streamin_propty);
                                  ...
                                  audio_source_t source __unused)
{
    ...
    ALOGD("audio_hal:usb adev_open_input_stream, proxy_config.channels=%d", proxy_config.channels);
    proxy_prepare(&in->proxy, in->profile, &proxy_config);
    ALOGD("audio_hal:usb adev_open_input_stream, in->proxy.channels=%d", in->proxy.alsa_config.channels);
