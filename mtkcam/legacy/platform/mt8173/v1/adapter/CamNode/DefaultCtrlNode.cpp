...
MVOID *DefaultCtrlNodeImpl::stopSmoothZoomThreadFunc(MVOID *arg)
{
    ALOGD("enter stopSmoothZoomThreadFunc");

    MINT32 popRatio = -1;
    Vector<MINT32> vPopRatio;
    DefaultCtrlNodeImpl *mThis = (DefaultCtrlNodeImpl *)arg;
    while( -1 != (popRatio = mThis->getIspSyncCtrl()->popEndZoomRatio()) )
    {
    ...
    while( ! vPopRatio.empty() && ! mThis->getCallbackZoom()->requestStop())
    {
        ALOGD("Push ratio(%d) for compensation", *(vPopRatio.end()-1));
        mThis->getIspSyncCtrl()->addZoomRatio(*(vPopRatio.end()-1));
        vPopRatio.erase(vPopRatio.end()-1);
    }
    //
    if(mThis->getIspSyncCtrl())
    {
       mThis->getIspSyncCtrl()->setCallbackZoom(
                        (MVOID*)mThis,
                        cbZoomFunc);
    }
    return NULL;
...
stopSmoothZoom()
{
    ...
    //
    // create thread to handle stop zoom
    pthread_create(&mStopZoomThread, NULL, stopSmoothZoomThreadFunc, this);
