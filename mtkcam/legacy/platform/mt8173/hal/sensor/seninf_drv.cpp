...
int SeninfDrvImp::init()  //kk test
{
    ...
    mpCSI2RxAnalog0RegAddr = mpCSI2RxAnalogRegStartAddrAlign;
    mpCSI2RxAnalog1RegAddr = mpCSI2RxAnalogRegStartAddrAlign + (0x1000/4);
    mpCSI2RxAnalog2RegAddr = mpCSI2RxAnalogRegStartAddrAlign + (0x2000/4);
...
)
{
    ...
    if(ncsi2En == 1) {  // enable CSI2
...
#endif
        // MT8173: 1.011v-400Mhz , 0.9v-338Mhz
        // ISP CLK = 400Mhz , settel delay count =  (x ns *  400 M )/1000
      ...
      /*if(Isp_clk == 317)
        ...
        LOG_MSG("TermDelay:%d SettleDelay:%d ClkTermDelay:%d Vsync:%d lane_num:%d NCSI2_en:%d HeaderOrder:%d SettleDelay(cnt):%d dpcm:%d\n",
...
int SeninfDrvImp::setSeninf4Parallel(unsigned int parallelEn,unsigned int inDataType)
{
    ...
    if(parallelEn == 1) {
        if ((inDataType == RAW_8BIT_FMT)||(inDataType == YUV422_FMT)||(inDataType == RGB565_MIPI_FMT)||
            ...
            // MT8173 doesn't have MIPI interface #2, it should not been set
            // otherwise, it will cause system hang
            /*
            temp = *(mpCSI2RxAnalog2RegAddr + (0x4C/4));//GPI*_IES = 0 for GPIO
            mt65xx_reg_writel(temp|0x1041041, mpCSI2RxAnalog2RegAddr + (0x4C/4));
            temp = *(mpCSI2RxAnalog2RegAddr + (0x50/4));//GPI*_IES = 0 for GPIO
