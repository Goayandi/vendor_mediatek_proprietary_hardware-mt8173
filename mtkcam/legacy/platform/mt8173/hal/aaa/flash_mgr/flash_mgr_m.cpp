...
int FlashMgrM::setAeFlashMode(int aeMode, int flashMode)
{
	...
	mFlash1Mode = fm;
	logI("setAeFlashMode ae_mode=%d flash_mode=%d f1_mode=%d",aeMode, flashMode, mFlash1Mode);
	setFlashMode(fm);
	return 0;
}
...
int FlashMgrM::getFlashState()
{
    ...
    {
        Mutex::Autolock lock(mLock);
        logI("getFlashState %d %d %d", mFlashStateStack[0], mFlashStateStack[1], mFlashStateStack[2]);
