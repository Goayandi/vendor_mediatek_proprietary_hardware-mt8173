...
getHwPortConfig(list<HwPortConfig_t>* plPortCfg)
{
	...
	MY_LOGD("---- Imgo:(w x h = %d x %d) ----", sensorSize.w, sensorSize.h );
    ...
    if( mbCfgImgo )
    {
		...
		MY_LOGD("---- Imgo:(w x h = %d x %d), fmt= 0x%x ----", full.mSize.w, full.mSize.h, fmt);
        if((full.mSize.w == 0) || (full.mSize.h == 0))
        {
			AEE_ASSERT("----Pass1 FULL Size ERROR----");
        }
    ...
    if( mbCfgRrzo )
    {
		...
		MY_LOGD("---- Rrzo:(w x h = %d x %d), fmt= 0x%x ----", resized.mSize.w, resized.mSize.h, fmt);
        if((resized.mSize.w == 0) || (resized.mSize.h == 0))
        {
			AEE_ASSERT("----Pass1 resized Size ERROR----");
        }
...
allocBuffers(list<HwPortConfig_t> & lPortCfg)
{
    ...
    while( pConfig != lPortCfg.end() )
    {
        ...
        for(MUINT32 i = 0; i < getRingBufCnt() ; i++ )
        {
		    ...
		    MY_LOGD("---- pConfig->mSize.w = %d, pConfig->mSize.h = %d", 
				     pConfig->mSize.w, pConfig->mSize.h);
