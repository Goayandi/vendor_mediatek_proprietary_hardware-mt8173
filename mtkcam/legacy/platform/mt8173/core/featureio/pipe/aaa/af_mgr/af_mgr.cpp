...
Mutex m_LockAfListenData;

void afListenHandler(ASensorEvent event)
{
    Mutex::Autolock lock(m_LockAfListenData);
    switch(event.type)
    {
...
#if MTKCAM_TEMP_AFMGR_AFALGO_CALL_INITAF 
#warning "MTKCAM_TEMP_AFMGR_AFALGO_CALL_INITAF is 1"
    m_pIAfAlgo->initAF(m_sAFInput, m_sAFOutput);
#else
#warning "MTKCAM_TEMP_AFMGR_AFALGO_CALL_INITAF is 0"
#endif
