...
getInstance()
{
    ...
    MY_LOGD("[Create] &mASDCount:%p &gsLock:%p", &mASDCount, &gsLock);
    int const oldCount = ::android_atomic_inc(&mASDCount);

    int g_oldCount = oldCount;
    if(g_oldCount < 0)
    {
        MY_LOGW("Warning! [Create] count:%d->%d pHalASD:%p", oldCount, mASDCount, pHalASD);
        g_oldCount = 0;
        mASDCount = 1;
        pHalASD = NULL;
    }

    MY_LOGD("[Create] g_oldCount:%d->%d pHalASD:%p", g_oldCount, mASDCount, pHalASD);
    if  ( 0 == oldCount )
    {
        if  ( ! pHalASD )
        {
...
{
    Mutex::Autolock _l(gsLock);
    MY_LOGD("[Delete] &mASDCount:%p &gsLock:%p", &mASDCount, &gsLock);
    int const oldCount = ::android_atomic_dec(&mASDCount);
    MY_LOGD("[Delete] count:%d->%d pHalASD:%p", oldCount, mASDCount, pHalASD);
