...
#if FAKE_LIB3A
#warning "****Fake Lib3A****"
static inline LSC_RESULT shading_transform(SHADIND_TRFM_CONF &shading_conf)
{
    // fake implementation
    return S_LSC_CONVERT_OK;
}
static inline LSC_RESULT shading_align_golden(SHADIND_ALIGN_CONF &trans)
{
    // fake implementation
    return S_LSC_CONVERT_OK;
}
static inline LSC_RESULT LscGaintoHWTbl(float *p_pgn_float, unsigned int *p_lsc_tbl, int grid_x, int grid_y, int RawImgW, int RawImgH, void* WorkBuf, int BufSize)
{
     // fake implementation
    return S_LSC_CONVERT_OK;
}
static inline LSC_RESULT LscSL2Calcu(TBL_INFO_T tbl_cal_info, int grid_x, int grid_y, int RawImgW, int RawImgH)
{
     // fake implementation
    return S_LSC_CONVERT_OK;
}
static inline void vLSC_PARAM_INIT(LSC_CAL_INI_PARAM_T a_rLSCCaliINIParam)
{
     // fake implementation
}
static inline void vLSC_Calibration_INIT(unsigned short* a_u1BufferAddr)
{
     // fake implementation
}
static inline void vLSC_Calibration_END(void)
{
     // fake implementation
}
static inline void vLsc_Remap_to_Bayer0(unsigned short *Src, unsigned short *Dst, unsigned int Width, unsigned int Height, int BayerOrder)
{
     // fake implementation
}
static inline MINT32 mrLSC_Calibrate(LSC_CALI_INFO_T cali_info, MUINT8 a_u1Mode, MUINT16 a_u2SVDTermNum)
{
     // fake implementation
    return 0;
}
...
#else
#warning "****Not Fake Lib3A, the real Lib3A****"
