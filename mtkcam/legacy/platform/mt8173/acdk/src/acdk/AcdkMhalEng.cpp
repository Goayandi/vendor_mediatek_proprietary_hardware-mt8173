...
/* Fix ME: define L1_CACHE_BYTES due to linux/cache.h is remove from bionic */
/* Hardcode here for mtkcam MW using , But it may not really L1 cache size  */
...
MINT32 AcdkMhalEng::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    if(mAcdkMhalPrvParam.sensorIndex == 0)
        ...
        AcdkMhalOpenDevice((hw_module_t *)mModule, "2");
...
//        set(map, "rawfname", "/data/nvram/");
        set(map, "rawfname", ACDK_RAW_FILE_PATH);
        switch(eImgType)
        {
            case PURE_RAW8_TYPE:
                set(map, "isp-mode", "1");
