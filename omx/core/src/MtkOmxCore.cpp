...
static bool ParseMtkCoreConfig(const char *cfgFileName)
{
    ...
    int i;
    int moffset = 0;
    char str[MAX_LINE_LEN*MAX_COMPONENT_INSTANCE]={0};//store all cfg
    ...
    while (!feof(fp))
    {
        //for coverity id: 126636  Ignoring number of bytes read (CHECKED_RETURN)
        mfileSize = fread(str, 1, MAX_LINE_LEN*MAX_COMPONENT_INSTANCE, fp);
        //MTK_OMX_LOGD("mfileSize %d", mfileSize);
        //if (fread(str, 1, MAX_LINE_LEN*MAX_COMPONENT_INSTANCE, fp) == NULL)
        if( mfileSize == 0 )
        {
            break;
        }
    ...
    //for coverity id: id: 127585: String not null terminated (STRING_NULL)
    if( 0 == strlen(str) )
    {
        MTK_OMX_LOGE("[ERROR] No core component file available");
        ret = false;
        goto EXIT;
    }

    //MTK_OMX_LOGD("len %d, strstr %s", strlen(str), str);
    pchOMX = strstr(str, MTKOMXTAG);
    //MTK_OMX_LOGD("pchOMX = %x, diff %d", pchOMX, pchOMX-str);
    pchEOL = strchr(str,'\n');
    //MTK_OMX_LOGD("pchEOL = %x, diff %d", pchEOL, pchEOL-str);
    while (pchOMX != NULL) {
        gCoreComponentCounts++;
        pchOMX = strstr(pchOMX+1,MTKOMXTAG);
        //MTK_OMX_LOGD("pchOMX = %x, diff %d", pchOMX, (pchOMX==NULL)?0:pchOMX-str);
        if( pchEOL == NULL )
        {
            break;
        }
        pchEOL = strchr(pchEOL+1,'\n');
    ...
    while (pchOMX != NULL)
    {
        //MTK_OMX_LOGD("2 diff = %d", (pchEOL==NULL)?(strlen(str)-moffset):(pchEOL-(str+moffset)));
        if (memcpy(str_line, str+moffset, (pchEOL==NULL)?(strlen(str)-moffset):(pchEOL-(str+moffset))) == NULL)
        {
            break;
        }
...
static int add_inst_count(char *name, int add)
{
    ...
    if (add > 0)
    {
        ...
        gCompInstanceCount[i].comp_name = (char *)malloc(strlen(name)+1);
        strcpy(gCompInstanceCount[i].comp_name, name);
    ...
    *pHandle = NULL;
    ...
    if (comp_index >= 0)
    {
        ...
        *pHandle = (*pMtkOmxComponentCreate)(cComponentName);
        ...
        gCoreCompInstance[free_inst_handle_index].comp_name = (char *)malloc(strlen(cComponentName)+1);
        strcpy(gCoreCompInstance[free_inst_handle_index].comp_name, cComponentName);
