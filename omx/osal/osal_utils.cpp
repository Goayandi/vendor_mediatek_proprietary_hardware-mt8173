...
int pthread_mutex_lock_timeout (pthread_mutex_t *mutex, const int millisec)
{
    ...
    while ((retcode = pthread_mutex_trylock (mutex)) == EBUSY) {
        if (nWaitMilliSec >= millisec) {
            ALOGE("pthread_mutex_lock_timeout timeout");
            return ETIMEDOUT;
        }

        if (nWaitMilliSec == 100)
            ALOGI("pthread_mutex_lock nWaitMilliSec : %d", nWaitMilliSec);
...
int sem_wait_timeout (sem_t *sem, const int millisec)
{
    int retcode = 0;
    int nWaitMilliSec = 0;

    while ((retcode = sem_trywait(sem)) == -1 && errno == EAGAIN) {
        if (nWaitMilliSec >= millisec) {
            ALOGE("sem_wait_timeout timeout");
            return ETIMEDOUT;
        }

        if (nWaitMilliSec == 100)
            ALOGI("sem_wait nWaitMilliSec : %d", nWaitMilliSec);
