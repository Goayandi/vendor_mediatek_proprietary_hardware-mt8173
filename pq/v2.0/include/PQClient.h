...
public:
    ...
    status_t enableBlueLight(bool enable);
    status_t getBlueLightEnabled(bool *isEnabled);
    status_t setBlueLightStrength(int32_t strength);
    status_t getBlueLightStrength(int32_t *strength);
    status_t setTuningField(int32_t module, int32_t field, int32_t value);
    status_t getTuningField(int32_t module, int32_t field, int32_t *value);
    unsigned int * getAshmem(void);
    status_t setGammaIndex(int32_t index);
    status_t getGammaIndex(int32_t *index);

    status_t setGlobalPQSwitch(int32_t globalPQSwitch);
    status_t getGlobalPQSwitch(int32_t *globalPQSwitch);
    status_t setGlobalPQStrength(uint32_t globalPQStrength);
    status_t getGlobalPQStrength(uint32_t *globalPQStrength);
    status_t getGlobalPQStrengthRange(uint32_t *globalPQStrengthRange);
    status_t getGlobalPQIndex(GLOBAL_PQ_INDEX_T *globalPQindex);
