#define LOG_TAG "IPQService"
#define MTK_LOG_ENABLE 1
#include <cutils/log.h>
#include "IPQService.h"

namespace android {

// client : proxy PQ class
class BpPQService : public BpInterface<IPQService>
{
public:
    BpPQService(const sp<IBinder>& impl) : BpInterface<IPQService>(impl)
    {
    }
    virtual status_t getTDSHPFlag(int32_t *TDSHPFlag)
    {
        Parcel data, reply;

        if (remote()->transact(PQ_GET_TDSHP_FLAG, data, &reply) != NO_ERROR) {
            ALOGE("getTDSHPFlag could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *TDSHPFlag = reply.readInt32();

        return ret;
    }
    virtual status_t setTDSHPFlag(int32_t TDSHPFlag)
    {
        Parcel data, reply;
        data.writeInt32(TDSHPFlag);
        if (remote()->transact(PQ_SET_TDSHP_FLAG, data, &reply) != NO_ERROR) {
            ALOGE("setTDSHPFlag could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }
    virtual status_t getColorRegion(DISP_PQ_WIN_PARAM *win_param)
    {
        Parcel data, reply;

        if (remote()->transact(PQ_GET_COLOR_REGION, data, &reply) != NO_ERROR) {
            ALOGE("getColorRegion could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        win_param->split_en= reply.readInt32();
        win_param->start_x= reply.readInt32();
        win_param->start_y= reply.readInt32();
        win_param->end_x= reply.readInt32();
        win_param->end_y= reply.readInt32();
        return ret;
    }

    virtual status_t setColorRegion(int32_t split_en,int32_t start_x,int32_t start_y,int32_t end_x,int32_t end_y)
    {
        Parcel data, reply;
        data.writeInt32(split_en);
        data.writeInt32(start_x);
        data.writeInt32(start_y);
        data.writeInt32(end_x);
        data.writeInt32(end_y);


        if (remote()->transact(PQ_SET_COLOR_REGION, data, &reply) != NO_ERROR) {
            ALOGE("setColorRegion could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t setPQMode(int32_t mode)
    {
        Parcel data, reply;
        data.writeInt32(mode);
        if (remote()->transact(PQ_SET_MODE, data, &reply) != NO_ERROR) {
            ALOGE("setPQMode could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t setPQIndex(int32_t level, int32_t scenario, int32_t tuning_mode,int32_t index)
    {
        Parcel data, reply;
        data.writeInt32(level);
        data.writeInt32(scenario);
        data.writeInt32(tuning_mode);
        data.writeInt32(index);
        if (remote()->transact(PQ_SET_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("setPQIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getMappedColorIndex(DISP_PQ_PARAM *index, int32_t scenario, int32_t mode)
    {
        Parcel data, reply;
        data.writeInt32(scenario);
        data.writeInt32(mode);
        if (remote()->transact(PQ_GET_MAPPED_COLOR_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("getMappedColorIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        index->u4SatGain = reply.readInt32();
        index->u4PartialY = reply.readInt32();
        index->u4HueAdj[0] = reply.readInt32();
        index->u4HueAdj[1] = reply.readInt32();
        index->u4HueAdj[2] = reply.readInt32();
        index->u4HueAdj[3] = reply.readInt32();
        index->u4SatAdj[0] = reply.readInt32();
        index->u4SatAdj[1] = reply.readInt32();
        index->u4SatAdj[2] = reply.readInt32();
        index->u4SatAdj[3] = reply.readInt32();
        index->u4Contrast = reply.readInt32();
        index->u4Brightness = reply.readInt32();
        return ret;
    }

    virtual status_t getMappedTDSHPIndex(DISP_PQ_PARAM *index, int32_t scenario, int32_t mode)
    {
        Parcel data, reply;
        data.writeInt32(scenario);
        data.writeInt32(mode);
        if (remote()->transact(PQ_GET_MAPPED_TDSHP_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("getMappedTDSHPIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        index->u4SHPGain = reply.readInt32();

        return ret;
    }

    virtual status_t setPQDCIndex(int32_t level, int32_t index)
    {
        Parcel data, reply;
        data.writeInt32(level);
        data.writeInt32(index);
        if (remote()->transact(PQ_SET_PQDC_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("setPQDCIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getPQDCIndex(DISP_PQ_DC_PARAM *dcparam, int32_t index)
    {
        Parcel data, reply;
        int32_t i;
        data.writeInt32(index);
        if (remote()->transact(PQ_GET_PQDC_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("getPQDCIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        for(i = 0; i < PQDC_INDEX_MAX; i++){
            dcparam->param[i] = reply.readInt32();
        }
        return ret;
    }

    virtual status_t getColorCapInfo(MDP_COLOR_CAP *param)
    {
        Parcel data, reply;
        int32_t i;
        if (remote()->transact(PQ_GET_COLOR_CAP, data, &reply) != NO_ERROR) {
            ALOGE("getColorCapInfo could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        param->en    = reply.readInt32();
        param->pos_x = reply.readInt32();
        param->pos_y = reply.readInt32();
        return ret;
    }

    virtual status_t getTDSHPReg(MDP_TDSHP_REG_EX *param)
    {
        Parcel data, reply;
        int32_t i;
        if (remote()->transact(PQ_GET_TDSHP_REG, data, &reply) != NO_ERROR) {
            ALOGE("getTDSHPReg could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        for(i = 0; i < TDSHP_REG_INDEX_MAX; i++){
            param->param[i] = reply.readInt32();
        }
        return ret;
    }

    virtual status_t getPQDSIndex(DISP_PQ_DS_PARAM_EX *dsparam)
    {
        Parcel data, reply;
        int32_t i;
        if (remote()->transact(PQ_GET_PQDS_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("getPQDSIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        for(i = 0; i < PQDS_INDEX_MAX; i++){
            dsparam->param[i] = reply.readInt32();
        }
        return ret;
    }

    virtual status_t getColorIndex(DISP_PQ_PARAM *index, int32_t scenario, int32_t mode)
    {
        Parcel data, reply;
        data.writeInt32(scenario);
        data.writeInt32(mode);
        if (remote()->transact(PQ_GET_COLOR_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("getColorIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        index->u4SatGain = reply.readInt32();
        index->u4PartialY= reply.readInt32();
        index->u4HueAdj[0] = reply.readInt32();
        index->u4HueAdj[1] = reply.readInt32();
        index->u4HueAdj[2] = reply.readInt32();
        index->u4HueAdj[3] = reply.readInt32();
        index->u4SatAdj[0] = reply.readInt32();
        index->u4SatAdj[1] = reply.readInt32();
        index->u4SatAdj[2] = reply.readInt32();
        index->u4SatAdj[3] = reply.readInt32();;
        index->u4Contrast = reply.readInt32();
        index->u4Brightness = reply.readInt32();
        return ret;
    }

    virtual status_t getTDSHPIndex(DISP_PQ_PARAM *index, int32_t scenario, int32_t mode)
    {
        Parcel data, reply;
        data.writeInt32(scenario);
        data.writeInt32(mode);
        if (remote()->transact(PQ_GET_TDSHP_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("getTDSHPIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        index->u4SHPGain = reply.readInt32();

        return ret;
    }
    virtual status_t setDISPScenario(int32_t scenario)
    {
        Parcel data, reply;
        data.writeInt32(scenario);

        if (remote()->transact(PQ_SET_SCENARIO, data, &reply) != NO_ERROR) {
            ALOGE("setDISPScenario could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t setFeatureSwitch(PQFeatureID id, uint32_t value)
    {
        Parcel data, reply;

        data.writeInt32(id);
        data.writeInt32(value);
        if (remote()->transact(PQ_SET_FEATURE_SWITCH, data, &reply) != NO_ERROR) {
            ALOGE("setFeatureSwitch could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getFeatureSwitch(PQFeatureID id, uint32_t *value)
    {
        Parcel data, reply;

        data.writeInt32(id);
        if (remote()->transact(PQ_GET_FEATURE_SWITCH, data, &reply) != NO_ERROR) {
            ALOGE("getFeatureSwitch could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *value = reply.readInt32();

        return ret;
    }

    virtual status_t enableBlueLight(bool enable)
    {
        Parcel data, reply;

        data.writeInt32(enable ? 1 : 0);

        if (remote()->transact(PQ_ENABLE_BLUE_LIGHT, data, &reply) != NO_ERROR) {
            ALOGE("enableBlueLight could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getBlueLightEnabled(bool *isEnabled)
    {
        Parcel data, reply;

        if (remote()->transact(PQ_GET_BLUE_LIGHT_ENABLED, data, &reply) != NO_ERROR) {
            ALOGE("getBlueLightEnabled could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *isEnabled = (reply.readInt32() != 0 ? true : false);

        return ret;
    }

    virtual status_t setBlueLightStrength(int32_t strength)
    {
        Parcel data, reply;

        data.writeInt32(strength);

        if (remote()->transact(PQ_SET_BLUE_LIGHT_STRENGTH, data, &reply) != NO_ERROR) {
            ALOGE("setBlueLightStrength could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getBlueLightStrength(int32_t *strength)
    {
        Parcel data, reply;

        if (remote()->transact(PQ_GET_BLUE_LIGHT_STRENGTH, data, &reply) != NO_ERROR) {
            ALOGE("getBlueLightStrength could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *strength = reply.readInt32();

        return ret;
    }

    virtual status_t setTuningField(int32_t module, int32_t field, int32_t value)
    {
        Parcel data, reply;

        data.writeInt32(module);
        data.writeInt32(field);
        data.writeInt32(value);
        if (remote()->transact(PQ_SET_TUNING_FIELD, data, &reply) != NO_ERROR) {
            ALOGE("setTuningField could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getTuningField(int32_t module, int32_t field, int32_t *value)
    {
        Parcel data, reply;

        data.writeInt32(module);
        data.writeInt32(field);
        if (remote()->transact(PQ_GET_TUNING_FIELD, data, &reply) != NO_ERROR) {
            ALOGE("getTuningField could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *value = reply.readInt32();

        return ret;
    }

    virtual sp<IMemoryHeap> getAshmem(void)
    {
        Parcel data, reply;

        sp<IMemoryHeap> memHeap = NULL;
        data.writeInterfaceToken(IPQService::getInterfaceDescriptor());
        remote()->transact(PQ_GET_ASHMEM, data, &reply);
        memHeap = interface_cast<IMemoryHeap> (reply.readStrongBinder());

        return memHeap;
    }

    virtual status_t setGammaIndex(int32_t index)
    {
        Parcel data, reply;

        data.writeInt32(index);
        if (remote()->transact(PQ_SET_GAMMA_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("setGammaIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getGammaIndex(int32_t *index)
    {
        Parcel data, reply;

        if (remote()->transact(PQ_GET_GAMMA_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("getGammaIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *index = reply.readInt32();

        return ret;
    }

    virtual status_t setGlobalPQSwitch(int32_t globalPQSwitch)
    {
        Parcel data, reply;

        data.writeInt32(globalPQSwitch);
        if (remote()->transact(PQ_SET_GLOBAL_PQ_SWITCH, data, &reply) != NO_ERROR) {
            ALOGE("setGlobalPQSwitch could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getGlobalPQSwitch(int32_t *globalPQSwitch)
    {
        Parcel data, reply;

        if (remote()->transact(PQ_GET_GLOBAL_PQ_SWITCH, data, &reply) != NO_ERROR) {
            ALOGE("getGlobalPQSwitch could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *globalPQSwitch = reply.readInt32();

        return ret;
    }

    virtual status_t setGlobalPQStrength(uint32_t globalPQStrength)
    {
        Parcel data, reply;

        data.writeInt32(globalPQStrength);
        if (remote()->transact(PQ_SET_GLOBAL_PQ_STRENGTH, data, &reply) != NO_ERROR) {
            ALOGE("setGlobalPQStrength could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        return ret;
    }

    virtual status_t getGlobalPQStrength(uint32_t *globalPQStrength)
    {
        Parcel data, reply;

        if (remote()->transact(PQ_GET_GLOBAL_PQ_STRENGTH, data, &reply) != NO_ERROR) {
            ALOGE("getGlobalPQStrength could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *globalPQStrength = reply.readInt32();

        return ret;
    }

    virtual status_t getGlobalPQStrengthRange(uint32_t *globalPQStrengthRange)
    {
        Parcel data, reply;

        if (remote()->transact(PQ_GET_GLOBAL_PQ_STRENGTH_RANGE, data, &reply) != NO_ERROR) {
            ALOGE("getGlobalPQStrengthRange could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());
        *globalPQStrengthRange = reply.readInt32();

        return ret;
    }

    virtual status_t getGlobalPQIndex(GLOBAL_PQ_INDEX_T *globalPQindex)
    {
        Parcel data, reply;
        int i, j;

        if (remote()->transact(PQ_GET_GLOBAL_PQ_INDEX, data, &reply) != NO_ERROR) {
            ALOGE("getGlobalPQIndex could not contact remote\n");
            return -1;
        }

        status_t ret = static_cast<status_t>(reply.readInt32());

        for(i = 0; i < GLOBAL_PQ_DC_INDEX_MAX; i++) {
            for (j = 0; j < 40; j++) {
                globalPQindex->dcindex.entry[i][j] = reply.readInt32();
            }
        }

        return ret;
    }

};


IMPLEMENT_META_INTERFACE(PQService, "PQService");

status_t BnPQService::onTransact(uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    status_t ret = 0;
    //ALOGD("receieve the command code %d", code);

    switch(code)
    {
        case PQ_SET_INDEX:
            {
                int32_t level;
                int32_t scenario;
                int32_t tuning_mode;
                int32_t index;

                data.readInt32(&level);
                data.readInt32(&scenario);
                data.readInt32(&tuning_mode);
                data.readInt32(&index);
                ret = setPQIndex(level,scenario,tuning_mode,index);

                reply->writeInt32(ret);

            }
            break;
        case PQ_GET_MAPPED_COLOR_INDEX:
            {
                DISP_PQ_PARAM  param;
                int32_t scenario;
                int32_t mode;
                data.readInt32(&scenario);
                data.readInt32(&mode);

                ret = getMappedColorIndex(&param,scenario,mode);

                reply->writeInt32(ret);
                reply->writeInt32(param.u4SatGain);
                reply->writeInt32(param.u4PartialY);
                reply->writeInt32(param.u4HueAdj[0]);
                reply->writeInt32(param.u4HueAdj[1]);
                reply->writeInt32(param.u4HueAdj[2]);
                reply->writeInt32(param.u4HueAdj[3]);
                reply->writeInt32(param.u4SatAdj[0]);
                reply->writeInt32(param.u4SatAdj[1]);
                reply->writeInt32(param.u4SatAdj[2]);
                reply->writeInt32(param.u4SatAdj[3]);
                reply->writeInt32(param.u4Contrast);
                reply->writeInt32(param.u4Brightness);
            }
            break;
        case PQ_GET_MAPPED_TDSHP_INDEX:
            {
                DISP_PQ_PARAM  param;
                int32_t scenario;
                int32_t mode;
                data.readInt32(&scenario);
                data.readInt32(&mode);

                ret = getMappedTDSHPIndex(&param,scenario,mode);

                reply->writeInt32(ret);
                reply->writeInt32(param.u4SHPGain);

            }
            break;
        case PQ_GET_COLOR_INDEX:
            {
                DISP_PQ_PARAM  param;
                int32_t scenario;
                int32_t mode;
                data.readInt32(&scenario);
                data.readInt32(&mode);

                ret = getColorIndex(&param,scenario,mode);

                reply->writeInt32(ret);
                reply->writeInt32(param.u4SatGain);
                reply->writeInt32(param.u4PartialY);
                reply->writeInt32(param.u4HueAdj[0]);
                reply->writeInt32(param.u4HueAdj[1]);
                reply->writeInt32(param.u4HueAdj[2]);
                reply->writeInt32(param.u4HueAdj[3]);
                reply->writeInt32(param.u4SatAdj[0]);
                reply->writeInt32(param.u4SatAdj[1]);
                reply->writeInt32(param.u4SatAdj[2]);
                reply->writeInt32(param.u4SatAdj[3]);
                reply->writeInt32(param.u4Contrast);
                reply->writeInt32(param.u4Brightness);
            }
            break;
        case PQ_SET_PQDC_INDEX:
            {
                int32_t level;
                int32_t index;

                data.readInt32(&level);
                data.readInt32(&index);

                ret = setPQDCIndex(level,index);

                reply->writeInt32(ret);

                // LumaAdj should be added  later

            }
            break;
        case PQ_GET_PQDC_INDEX:
            {
                DISP_PQ_DC_PARAM dcparam;
                int32_t index;
                int32_t i;
                data.readInt32(&index);

                ret = getPQDCIndex(&dcparam,index);

                reply->writeInt32(ret);

                for(i = 0; i < PQDC_INDEX_MAX; i++){
                    reply->writeInt32(dcparam.param[i]);
                }

                // LumaAdj should be added  later

            }
            break;
        case PQ_GET_PQDS_INDEX:
            {
                DISP_PQ_DS_PARAM_EX dsparam;
                int32_t i;

                ret = getPQDSIndex(&dsparam);

                reply->writeInt32(ret);

                for(i = 0; i < PQDS_INDEX_MAX; i++){
                    reply->writeInt32(dsparam.param[i]);
                }

            }
            break;
        case PQ_GET_TDSHP_INDEX:
            {
                DISP_PQ_PARAM  param;
                int32_t scenario;
                int32_t mode;
                data.readInt32(&scenario);
                data.readInt32(&mode);

                ret = getTDSHPIndex(&param,scenario,mode);

                reply->writeInt32(ret);
                reply->writeInt32(param.u4SHPGain);

            }
            break;
        case PQ_GET_COLOR_CAP:
            {
                MDP_COLOR_CAP param;

                ret = getColorCapInfo(&param);

                reply->writeInt32(ret);
                reply->writeInt32(param.en);
                reply->writeInt32(param.pos_x);
                reply->writeInt32(param.pos_y);
            }
            break;
        case PQ_GET_TDSHP_REG:
            {
                MDP_TDSHP_REG_EX param;
                int32_t i;

                ret = getTDSHPReg(&param);

                reply->writeInt32(ret);

                for(i = 0; i < TDSHP_REG_INDEX_MAX; i++){
                    reply->writeInt32(param.param[i]);
                }
            }
            break;
        case PQ_SET_MODE:
            {
                int32_t mode;
                data.readInt32(&mode);

                ret = setPQMode(mode);

                reply->writeInt32(ret);

            }
            break;
        case PQ_SET_COLOR_REGION:
            {
                int32_t split_en;
                int32_t start_x;
                int32_t start_y;
                int32_t end_x;
                int32_t end_y;
                data.readInt32(&split_en);
                data.readInt32(&start_x);
                data.readInt32(&start_y);
                data.readInt32(&end_x);
                data.readInt32(&end_y);
                ret = setColorRegion(split_en,start_x,start_y,end_x,end_y);

                reply->writeInt32(ret);

            }
            break;
        case PQ_GET_COLOR_REGION:
            {
                DISP_PQ_WIN_PARAM param;
                ret = getColorRegion(&param);

                reply->writeInt32(ret);
                reply->writeInt32(param.split_en);
                reply->writeInt32(param.start_x);
                reply->writeInt32(param.start_y);
                reply->writeInt32(param.end_x);
                reply->writeInt32(param.end_y);
            }
            break;
        case PQ_GET_TDSHP_FLAG:
            {
                int32_t tdshp_flag;
                ret = getTDSHPFlag(&tdshp_flag);

                reply->writeInt32(ret);
                reply->writeInt32(tdshp_flag);
            }
            break;
        case PQ_SET_TDSHP_FLAG:
            {
                int32_t tdshp_flag;
                 data.readInt32(&tdshp_flag);
                ret = setTDSHPFlag(tdshp_flag);

                reply->writeInt32(ret);
            }
            break;
        case PQ_SET_SCENARIO:
            {
                int32_t scenario;
                data.readInt32(&scenario);
                ret = setDISPScenario(scenario);

                reply->writeInt32(ret);
            }
            break;

        case PQ_SET_FEATURE_SWITCH:
            {

                int32_t id;
                int32_t value;
                data.readInt32(&id);
                data.readInt32(&value);

                ret = setFeatureSwitch(static_cast<PQFeatureID>(id), value);

                reply->writeInt32(ret);
            }
            break;

        case PQ_GET_FEATURE_SWITCH:
            {
                int32_t id;
                uint32_t value;
                data.readInt32(&id);

                ret = getFeatureSwitch(static_cast<PQFeatureID>(id), &value);

                reply->writeInt32(ret);
                reply->writeInt32(value);
            }
            break;

        case PQ_ENABLE_BLUE_LIGHT:
            {
                int32_t isEnabled;

                data.readInt32(&isEnabled);
                ret = enableBlueLight(isEnabled ? true : false);
                reply->writeInt32(ret);
            }
            break;

        case PQ_GET_BLUE_LIGHT_ENABLED:
            {
                bool isEnabled;

                ret = getBlueLightEnabled(&isEnabled);
                reply->writeInt32(ret);
                reply->writeInt32(isEnabled ? 1 : 0);
            }
            break;

        case PQ_SET_BLUE_LIGHT_STRENGTH:
            {
                int32_t strength;

                data.readInt32(&strength);
                ret = setBlueLightStrength(strength);
                reply->writeInt32(ret);
            }
            break;

        case PQ_GET_BLUE_LIGHT_STRENGTH:
            {
                int32_t strength;

                ret = getBlueLightStrength(&strength);
                reply->writeInt32(ret);
                reply->writeInt32(strength);
            }
            break;

        case PQ_SET_TUNING_FIELD:
            {
                int32_t module, field, value;

                data.readInt32(&module);
                data.readInt32(&field);
                data.readInt32(&value);
                ret = setTuningField(module, field, value);
                reply->writeInt32(ret);
            }
            break;

        case PQ_GET_TUNING_FIELD:
            {
                int32_t module, field, value;

                data.readInt32(&module);
                data.readInt32(&field);
                ret = getTuningField(module, field, &value);
                reply->writeInt32(ret);
                reply->writeInt32(value);
            }
            break;

       case PQ_GET_ASHMEM:
            {
                sp<IMemoryHeap> Data = getAshmem();

                if (Data != NULL)
                {
                    reply->writeStrongBinder(IInterface::asBinder(Data));
                }
            }
            break;

       case PQ_SET_GAMMA_INDEX:
            {
                int32_t index;

                data.readInt32(&index);
                ret = setGammaIndex(index);
                reply->writeInt32(ret);
            }
            break;

       case PQ_GET_GAMMA_INDEX:
            {
                int32_t index;

                ret = getGammaIndex(&index);
                reply->writeInt32(ret);
                reply->writeInt32(index);
            }
            break;

       case PQ_SET_GLOBAL_PQ_SWITCH:
            {
                int32_t globalPQSwitch;

                data.readInt32(&globalPQSwitch);
                ret = setGlobalPQSwitch(globalPQSwitch);
                reply->writeInt32(ret);
            }
            break;

       case PQ_GET_GLOBAL_PQ_SWITCH:
            {
                int32_t globalPQSwitch;
                ret = getGlobalPQSwitch(&globalPQSwitch);
                reply->writeInt32(ret);
                reply->writeInt32(globalPQSwitch);
            }
            break;

       case PQ_SET_GLOBAL_PQ_STRENGTH:
            {
                uint32_t globalPQStrength;

                data.readUint32(&globalPQStrength);
                ret = setGlobalPQStrength(globalPQStrength);
                reply->writeInt32(ret);
            }
            break;

       case PQ_GET_GLOBAL_PQ_STRENGTH:
            {
                uint32_t globalPQStrength;
                ret = getGlobalPQStrength(&globalPQStrength);
                reply->writeInt32(ret);
                reply->writeInt32(globalPQStrength);
            }
            break;

       case PQ_GET_GLOBAL_PQ_STRENGTH_RANGE:
            {
                uint32_t globalPQStrengthRange;
                ret = getGlobalPQStrengthRange(&globalPQStrengthRange);
                reply->writeInt32(ret);
                reply->writeInt32(globalPQStrengthRange);
            }
            break;

       case PQ_GET_GLOBAL_PQ_INDEX:
            {
                GLOBAL_PQ_INDEX_T globalPQindex;
                int32_t i, j;

                ret = getGlobalPQIndex(&globalPQindex);

                reply->writeInt32(ret);
                for (i = 0; i < GLOBAL_PQ_DC_INDEX_MAX; i++) {
                    for (j = 0; j < 40; j++) {
                        reply->writeInt32(globalPQindex.dcindex.entry[i][j]);
                    }
                }
            }
            break;

        default:
            return BBinder::onTransact(code, data, reply, flags);
    }
    return ret;
}
};

