...
status_t PQClient::getPQDSIndex(DISP_PQ_DS_PARAM_EX *dsparam)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    err = assertStateLocked();
...
status_t PQClient::getTDSHPReg(MDP_TDSHP_REG_EX *param)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    err = assertStateLocked();
...
status_t PQClient::enableBlueLight(bool enable)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->enableBlueLight(enable);

    return err;

}

status_t PQClient::getBlueLightEnabled(bool *isEnabled)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->getBlueLightEnabled(isEnabled);

    return err;
}

status_t PQClient::setBlueLightStrength(int32_t strength)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->setBlueLightStrength(strength);

    return err;
}

status_t PQClient::getBlueLightStrength(int32_t *strength)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->getBlueLightStrength(strength);

    return err;
}

status_t PQClient::setTuningField(int32_t module, int32_t field, int32_t value)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->setTuningField(module, field, value);

    return err;
}

status_t PQClient::getTuningField(int32_t module, int32_t field, int32_t *value)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->getTuningField(module, field, value);

    return err;
}

unsigned int * PQClient::getAshmem(void)
{
    static sp<IMemoryHeap> receiverMemBase;
    Mutex::Autolock _l(mLock);

    receiverMemBase = mPQService->getAshmem();

    return (unsigned int *) receiverMemBase->getBase();
}

status_t PQClient::setGammaIndex(int32_t index)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->setGammaIndex(index);

    return err;
}

status_t PQClient::getGammaIndex(int32_t *index)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->getGammaIndex(index);

    return err;
}

status_t PQClient::setGlobalPQSwitch(int32_t globalPQSwitch)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->setGlobalPQSwitch(globalPQSwitch);

    return err;
}

status_t PQClient::getGlobalPQSwitch(int32_t *globalPQSwitch)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->getGlobalPQSwitch(globalPQSwitch);

    return err;
}

status_t PQClient::setGlobalPQStrength(uint32_t globalPQStrength)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->setGlobalPQStrength(globalPQStrength);

    return err;
}

status_t PQClient::getGlobalPQStrength(uint32_t *globalPQStrength)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->getGlobalPQStrength(globalPQStrength);

    return err;
}

status_t PQClient::getGlobalPQStrengthRange(uint32_t *globalPQStrengthRange)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->getGlobalPQStrengthRange(globalPQStrengthRange);

    return err;
}

status_t PQClient::getGlobalPQIndex(GLOBAL_PQ_INDEX_T *globalPQindex)
{
    status_t err;
    Mutex::Autolock _l(mLock);
    ...
    err = assertStateLocked();
    if (err == NO_ERROR)
        err = mPQService->getGlobalPQIndex(globalPQindex);
