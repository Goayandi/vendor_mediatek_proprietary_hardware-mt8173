...
#define MTK_FLP2HAL   "mtk_flp2hal"
#define MTK_HAL2FLP   "mtk_hal2flp"
#define MAX_UDP_PAYLOAD_LEN    600
...
{
    FLP_TIMER_ID_PSEUDO,
    FLP_TIMER_ID_GEOFENCE,
    FLP_TIMER_ID_RESERVED,
    FLP_TIMER_ID_CHECKCNN,
    FLP_TIMER_ID_MAX
} MTK_FLP_TIMER_E;
...
typedef struct
{
    UINT32 type;
    UINT32 length;
    UINT8 payload[MAX_UDP_PAYLOAD_LEN];
} MTK_FLP_UDP_MSG_T;
...
#define MTK_FLP_MSG_RING_SIZE 100
...
{
    INT32 sys_mode;
    INT32 init_mnld_flag;
}MTK_FLP_RECOVERY_INFO_T;
...
typedef enum {
    FLP_BATCH_NONE = 0x00,
    FLP_BATCH_GEOFENCE_ONLY = 0x03
}MTK_FLP_BATCH_MODE_T;

typedef enum{
    FLP_AP_MODE = 0x00,
    FLP_OFFLOAD_MODE = 0x01,
}MTK_FLP_SYS_MODE_T;
