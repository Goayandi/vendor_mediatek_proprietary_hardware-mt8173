...
VOID *mtk_flp_socket_thread(VOID * arg) {
    ...
    MTK_FLP_MSG_T *flp_msg, *mode_msg;
    ...
    hal2flp_fp = mtk_flp_sys_udp_socket_bind(MTK_HAL2FLP);

    /* request mode */
    FLPD("flp_socket_thread, FLPD reboot done ntf\n");
    mode_msg = mtk_flp_sys_msg_alloc(sizeof(MTK_FLP_MSG_T));
    mode_msg->type = MTK_FLP_MSG_SYS_FLPD_REBOOT_DONE_NTF;
    mode_msg->length = 0;
    mtk_flp_sys_udp_send_msg(MTK_FLP2HAL, mode_msg);
    mtk_flp_sys_msg_free(mode_msg);

    while(!g_ThreadExitSocket) {
        // - recv msg from socket interface
        ret = mtk_flp_sys_udp_recv_msg(hal2flp_fp, &flp_msg);
