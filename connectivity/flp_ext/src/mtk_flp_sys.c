...
// timer pool
static flp_timer_table_struct flp_timer_table[FLP_TIMER_ID_MAX];
...
MTK_FLP_MUTEX MTK_FLP_TIMER_MUTEX;
...
UINT32 mtk_flp_sys_get_time_tick() {
        struct timeval t;
        gettimeofday(&t, NULL);
        return t.tv_sec;
}

int mtk_flp_sys_udp_socket_bind(const char* path) {
    int fd;
    struct sockaddr_un addr;

    fd = socket(PF_LOCAL, SOCK_DGRAM, 0);
    if(fd < 0) {
        FLPD("mtk_flp_sys_udp_socket_bind() socket() failed reason=[%s]%d",
            strerror(errno), errno);
        return -1;
    }
    FLPD("mtk_flp_sys_udp_socket_bind fd=%d",fd);

    memset(&addr, 0, sizeof(addr));
    // an abstract unix local socket address
    addr.sun_path[0] = 0;
    memcpy(addr.sun_path + 1, path, strlen(path));
    addr.sun_family = AF_UNIX;
    unlink(addr.sun_path);
    if(bind(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        FLPD("mtk_flp_sys_udp_socket_bind() bind() failed path=[%s] reason=[%s]%d",
            path, strerror(errno), errno);
        close(fd);
        return -1;
    }
    FLPD("mtk_flp_sys_udp_socket_bind path=%s",path);
    if(addr.sun_path[0] != 0) {
        if(chmod(path, 0660) < 0) {
            FLPD("chmod err = [%s]", strerror(errno));
        }
    }
    return fd;
}

static int mtk_flp_sys_udp_send(const char* path, const char* buff, int len) {
    int ret = 0;
    struct sockaddr_un addr;
    int retry = 10;
    int fd = socket(PF_LOCAL, SOCK_DGRAM, 0);
    if(fd < 0) {
        FLPD("mtk_flp_sys_udp_sendto() socket() failed reason=[%s]%d",
            strerror(errno), errno);
        return -1;
    }
    FLPD("mtk_flp_sys_udp_sendto fd=%d len=%d",fd, len);
    ...
    while((ret = sendto(fd, buff, len, 0,
        ...
        FLPD("mtk_flp_sys_udp_sendto() sendto() failed path=[%s] ret=%d reason=[%s]%d",
...
static int mtk_flp_sys_udp_recv(int sockfd, char* buf, int len) {
    int ret = 0;
    int retry = 10;

    while((ret = recvfrom(sockfd, buf, len, 0,
         NULL, NULL)) == -1) {
        FLPD("ret=%d len=%d", ret, len);
        if(errno == EINTR) continue;
        if(errno == EAGAIN) {
            if(retry-- > 0) {
                usleep(100 * 1000);
        ...
        FLPD("mtk_flp_sys_udp_recvfrom() sendto reason=[%s]", strerror(errno));
        break;
    }
    return ret;
}

int mtk_flp_sys_udp_send_msg(const char* path, MTK_FLP_MSG_T *p_msg) {
    int i4SendLen;
    int ret;
    ...
    FLPD("Socket %s send msg: type:0x%02x, length, %d\n", path, p_msg->type, p_msg->length);
    ...
    if((p_msg->type == MTK_FLP_MSG_HAL_REPORT_LOC_NTF) || (p_msg->type == MTK_FLP_MSG_KER_REPORT_LOC_NTF) || (p_msg->type == MTK_FLP_MSG_DC_REPORT_LOC_NTF)){
        if(mtk_flp_sys_dbg_check_loc((FlpLocation *)p_msg + sizeof(MTK_FLP_MSG_T)) == FALSE ) {
            FLPE("Sending socket with wrong location data");
        }
    }
    ret = mtk_flp_sys_udp_send(path, (void *)p_msg, sizeof(MTK_FLP_MSG_T) + p_msg->length);
    return ret;
}

int mtk_flp_sys_udp_recv_msg(INT32 sockfd, MTK_FLP_MSG_T **p_msg) {
    int i4ReadLen = 0;
    MTK_FLP_UDP_MSG_T buff;

    //FLPD("Socket fd[%d] receiving...\n", sockfd);
    // read msg (blocking read)
    i4ReadLen = mtk_flp_sys_udp_recv(sockfd, (char*)&buff, sizeof(buff));
    if(i4ReadLen <= 0) {
        return MTK_FLP_ERROR;
    } else if(i4ReadLen != (int)(FLP_MSG_HDR_SIZE + buff.length)) {
        FLPD("unexpected length (read len: %d, exp len: %d)\n", i4ReadLen, (FLP_MSG_HDR_SIZE + buff.length));
        return MTK_FLP_ERROR;
    } else {
        FLPD("msg recv type: 0x%02x, len: %d\n", buff.type, buff.length);
        // malloc msg
        *p_msg = (MTK_FLP_MSG_T *)mtk_flp_sys_mem_alloc(FLP_MSG_HDR_SIZE + buff.length);
        if(*p_msg == NULL) {
            FLPD("malloc fail\n");
            return MTK_FLP_ERROR;
        }
        // fill type & length
        memcpy((unsigned char *)*p_msg, (unsigned char *)&buff, FLP_MSG_HDR_SIZE + buff.length);
    }
...
VOID *mtk_flp_sys_mem_alloc(UINT32 u4Size) {
    ...
    if(u4Size != 0) {
        ...
        FLPD("FLP_AP_ALOC: 0x%08X, %d", pMem, u4Size);
    }
    return pMem;
}
 ...
 *  MTK_FLP_SUCCESS
 *****************************************************************************/
INT32 mtk_flp_sys_mutex_create(MTK_FLP_MUTEX *mutex) {
 ...
 *  MTK_FLP_SUCCESS
 *****************************************************************************/
INT32 mtk_flp_sys_mutex_take(MTK_FLP_MUTEX *mutex) {
 ...
 *  MTK_FLP_SUCCESS
 *****************************************************************************/
INT32 mtk_flp_sys_mutex_give(MTK_FLP_MUTEX *mutex) {
 ...
 *  MTK_FLP_SUCCESS
 *****************************************************************************/
INT32 mtk_flp_sys_mutex_destory(MTK_FLP_MUTEX *mutex) {
 ...
 *  MTK_FLP_SUCCESS
 *****************************************************************************/
INT32 mtk_flp_sys_mutex_deinitialize(void) {
    return MTK_FLP_SUCCESS;
}
 ...
 *  MTK_FLP_SUCCESS
 *****************************************************************************/
INT32 mtk_flp_sys_msg_initialize(MTK_FLP_TASKID_E task_id) {
...
#if 1   //trace location
    loc = (FlpLocation *)(((INT8*)msg)+sizeof(MTK_FLP_MSG_T));
...
*/
UINT8 mtk_flp_sys_dbg_check_loc(FlpLocation *loc) {
    if(loc->size < sizeof(FlpLocation)) {
        FLPE("Wrong location size:%d, %d", loc->size, sizeof(FlpLocation));
        return FALSE;
    }
...
UINT8 mtk_flp_sys_dbg_check_locs(FlpLocation **locs, int num)
{
    int i;
    for(i = 0; i < num; i++) {
...
VOID mtk_flp_sys_dbg_dump_loc(FlpLocation *loc) {
...
int mtk_flp_load_sys_mode() {
    const char flp_prop_path[] = "/data/misc/flp.prop";
    char propbuf[512];
    FILE *fp = fopen(flp_prop_path, "rb");
    if(!fp) {
        FLPD("mtk_flp_load_sys_mode - open fd fail!");
        return FLP_AP_MODE;
    }
    while(fgets(propbuf, sizeof(propbuf), fp)) {
        if(strstr(propbuf, "flp.mode=ap")) {
            FLPD("mtk_flp_load_sys_mode - ap mode");
            return FLP_AP_MODE;
        } else if(strstr(propbuf, "flp.mode=offload")) {
            FLPD("mtk_flp_load_sys_mode - offload mode");
            return FLP_OFFLOAD_MODE;
        }
    }
    return FLP_AP_MODE;
}
...
VOID *mtk_flp_sys_uart_init(const CH* strDevPortName) {
...
INT32 mtk_flp_sys_uart_read(UINT8 *pBuffer, UINT32 nNbBytesToRead) {
    DWORD dwRead = 0;
...
INT32 mtk_flp_sys_uart_write(UINT8 *pBuffer, UINT32 nNbBytesToWrite) {
    DWORD dwWritten;
    UINT32 u4Offset = 0;
...
VOID flp_timer_expiry_hdlr(int sig, siginfo_t *si, void *uc) {
    ...
    /* Look up timer_slot of this timeout, range = 0 ~ (FLP_TIMER_ID_MAX-1) */
    for(timer_slot = 0; timer_slot < FLP_TIMER_ID_MAX; timer_slot++) {
    ...
    if(timer_slot == FLP_TIMER_ID_MAX) {
...
static VOID flp_timer_expiry_hdlr(union sigval sv) {
    ...
    mtk_flp_sys_mutex_take(&MTK_FLP_TIMER_MUTEX);

    timer_slot = (INT32)(sv.sival_int);

    if(timer_slot >= FLP_TIMER_ID_MAX) {
        FLPD("[TIMER]invalid timer_slot,%d\r\n", timer_slot);
        mtk_flp_sys_mutex_give(&MTK_FLP_TIMER_MUTEX);
        return;
    }
 ...
 *****************************************************************************/
INT32 mtk_flp_sys_timer_init(VOID) {
    int ret = MTK_FLP_SUCCESS;
    int timer_slot;

    mtk_flp_sys_mutex_create(&MTK_FLP_TIMER_MUTEX);
...
UINT32 mtk_flp_sys_timer_create(MTK_FLP_TIMER_E selected_timer) {
    ...
    if(selected_timer >= FLP_TIMER_ID_MAX) {
    ...
    if(flp_timer_table[timer_slot].is_used == TRUE) {
        ...
        mtk_flp_sys_mutex_give((MTK_FLP_MUTEX *)(MTK_FLP_MUTEX_TIMER));
        return timer_slot;
    }
...
VOID mtk_flp_sys_timer_start(UINT32 timer_slot, UINT32 period, ppCallBck_t timer_expiry, VOID *arg) {
    ...
    its.it_interval.tv_nsec = 1000000 * (period % 1000);
 ...
 *  NONE
 *****************************************************************************/
VOID mtk_flp_sys_timer_stop(MTK_FLP_TIMER_E timer_slot) {
 ...
 *  NONE
 *****************************************************************************/
VOID mtk_flp_sys_timer_delete(MTK_FLP_TIMER_E timer_slot) {
    UNUSED(timer_slot);
#if 0
    if (timer_slot >= FLP_TIMER_ID_MAX)
    {
