            ...
            if (0 <= (patch_fd = (open(patch_full_name, O_RDONLY)))) {
                ...
                if (-1 != lseek(patch_fd, 22, SEEK_SET)) {
                    ...
                    if (0 == ((patch_ver ^ fw_version) & 0x00ff)) {
                        ...
                        if (launcher_set_patch_info(g_wmt_fd, patch_info, patch_full_name, is_first)) {
...
static void launcher_combine_cfg_name(int chip_id, char *cfg_file_path)
{
    ...
    if ((0 <= index) && (NULL != cfg_file_path)) {
        ...
        strncpy(g_wmt_cfg_name, cfg_file_path, sizeof(g_wmt_cfg_name) - 1);
        g_wmt_cfg_name[sizeof(g_wmt_cfg_name) - 1] = '\0';
        strcat(g_wmt_cfg_name, "/");
        strcat(g_wmt_cfg_name, g_chip_mode_info[index].ant_mode.cfg_item_value);
        g_wmt_cfg_name[strlen(cfg_file_path) + strlen("/") +
            strlen(g_chip_mode_info[index].ant_mode.cfg_item_value)] = '\0';
    } else
        memset(g_wmt_cfg_name, 0, sizeof(g_wmt_cfg_name));
