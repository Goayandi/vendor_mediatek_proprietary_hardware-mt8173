...
#define CFG_GET_CHIPID_BY_PORPERTY            1
...
#define COMBO_IOC_ANT_IC_CHIP_ID            10
...
ANTStatus ant_set_package_title(ANT_U8 *pucMesg, ANT_U8 ucLen ,int dataType, ANT_U8 *pOutMesg)
{
     ANT_FUNC_START();
...
static int download_ANT_firmware(ant_channel_info_t *pstChnlInfo)
{
    ...
    int res;
#if CFG_GET_CHIPID_BY_PORPERTY
    int get_time = 20;
    char chip_id_str[PROPERTY_VALUE_MAX];
#endif

    ANT_DEBUG_I("get chip id\n");

#if CFG_GET_CHIPID_BY_PORPERTY
    while ((get_time-- != 0) && ((res = property_get("persist.mtk.wcn.combo.chipid", chip_id_str, NULL)) < 6)) {
        ANT_ERROR("get chip id fail, retry");
        usleep(200000);
    }
/* chip id is like "0xXXXX"*/
    if (res < 6) {
        ANT_ERROR("combo_chip_id error: %s\n", chip_id_str);
        return -1;
    }
    ANT_DEBUG_I("combo_chip_id is %s\n", chip_id_str);
    if (strcmp(chip_id_str, "0x6630") == 0)
        chip_id = 0x6630;
    else if (strcmp(chip_id_str, "0x6797") == 0)
        chip_id = 0x6797;
    else
        ANT_ERROR("Unknown chip id,download CONN ram-code by default!\n");
#else
    if (ioctl(pstChnlInfo->iFd, COMBO_IOC_ANT_IC_CHIP_ID, &chip_id) < 0) {
        ANT_ERROR("get COMBO_IOC_ANT_IC_CHIP_ID failed\n");
        return -2;
    }
    ANT_DEBUG_I("combo_chip_id is 0x%x\n", chip_id);
#endif

    if (0x6630 == chip_id) {
        ANT_DEBUG_I("query hw version setup\n");
        if (ioctl(pstChnlInfo->iFd, COMBO_IOC_ANT_IC_HW_VERSION, &hw_ver) < 0)
        {
            ANT_ERROR("get COMBO_IOC_ANT_IC_HW_VERSION failed\n");
            return -3;
        }
        ANT_DEBUG_I("query version: hw: 0x%x!\n",hw_ver);
        if (hw_ver == 0x8A00) {
            chip_ver = ANT_RAM_CODE_COMBO_V1;
        } else if (hw_ver >= 0x8A10) {
            chip_ver = ANT_RAM_CODE_COMBO_V2;
        } else {
            ANT_ERROR("Unknown chip hw version,download default ram-code!\n");
            chip_ver = ANT_RAM_CODE_COMBO_V2;
        }
    } else {
        ANT_DEBUG_I("download CONN ram-code!\n");
        chip_ver = ANT_RAM_CODE_CONN_V1;
    }

    res = ioctl(pstChnlInfo->iFd, COMBO_IOCTAL_ANT_DOWNLOAD_FIRMWARE, chip_ver);
    if(res == 0)
        ANT_DEBUG_I("download firmware ok\n");
    else
...
int ant_enable(void)
{
   ...
   for (eChannel = 0; eChannel < NUM_ANT_CHANNELS; eChannel++) {
        ...
        if(iRet){
            ANT_ERROR("download_ANT_firmware failed (%d)!\n", iRet);
            goto out;
        }
   }
